...
| Lighting | 🟢 IN_PROGRESS | `PLAN.md` R0.6: per-vertex lit/literal decision is data-driven via the real `G_MW_LIGHTCOL` signal (RE-103/RE-105); RE-164 retains stage X/Y angles and raw normals; RE-165 applies direct and MObj-originated LIGHT_1/LIGHT_2 writes, while RE-166 preserves their zero-valid presence and transitions under identical material flags; GE lighting is scoped to fighter draws and disabled for literal primitives | PPSSPP software capture confirms the v23 EBOOT/pack run at 60 FPS without errors; original-game visual comparison remains required before any correctness claim |
