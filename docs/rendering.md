...
| Effects / particles | IN PROGRESS | RE-172–RE-178 cover all 53 manager `EFDesc` records and all 46 unique display-bearing manager assets, including transform and material animation playback. `romtool effects` reports 24/26 material-animation tables replayable; PikachuUnk's table address is fixed, while ImpactWave and MBallThrown are confirmed source-unreachable rather than converter failures. Live colour-track GE state is resolved and unit-tested but not yet consumed by the renderer. Facing-dependent alternate streams and the separate `LBParticle` bytecode/texture-bank renderer also remain |
