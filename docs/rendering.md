...
| Texture decode | 🟢 85% | `PLAN.md` R0.3 `COMPLETE`: RGBA16/32, IA4/8/16, I4/8, CI4/8 decoded, unit-tested; 681/707 bound textures packed (RE-172) | 26 unconverted runtime-framebuffer references, each owned by R0.13 |
...
| Effects / particles | IN PROGRESS | RE-172–RE-179 cover all 53 manager `EFDesc` records and all 46 unique display-bearing manager assets, including transform, material, texture and live colour playback. `romtool effects` reports 24/26 material-animation tables replayable; PikachuUnk's table address is fixed, while ImpactWave and MBallThrown are confirmed source-unreachable rather than converter failures. RE-179 maps animated PRIM/ENV/LIGHT_1/LIGHT_2 onto the existing source-classified PSP paths and verifies the frame-4 result across all 26 material-animated manager effects; the manager census contains no animated BLEND track. RE-180 strictly decodes all nine independent `LBParticle` banks: 160 scripts, 65 texture series, 246 image frames and 6,070 used bytecode bytes. RE-181 packs the complete bank/script/series inventory and all 246 PSP-converted frames in version 25; `romtool effects` validates counts, bytecode, descriptors, frames and texel ranges after readback. RE-182 adds a deterministic host-side single-particle bytecode interpreter. RE-183 adds PSP-side drawing (`meshdraw::draw_particle`, a screen-aligned quad through the GE's own 3D pipeline standing in for the RDP's manual screen-space rectangle) and a debug-viewer proof of concept verified on script 0 at 60 FPS. RE-184 extends that settle point (spawn, tick x4) into a host-side census over all 160 real scripts: 148/160 resolve a texture frame; the other 12 are classified (1 authored zero-frame texture, 10 spawner-only scripts pending `LBGenerator`, 1 real particle sampled at a periodic zero size) rather than left as an unexplained gap. Facing-dependent manager streams, multi-particle spawn-tree execution, the `LBGenerator` subsystem, an on-device per-script screenshot sweep, and a real spawn event all remain |
...
| Framebuffer effects | COMPLETE for R0.13's own scope | `PLAN.md` R0.13: LB-transition capture/binding are implemented; all 13 files are visually verified (RE-116); RE-146/147 resolve, pack and replay all 11 wipes; RE-148 adds the synchronized capture-before-results lifecycle and PPSSPP-verifies Aeroplane at 60 FPS; RE-149 records the renderer/gameplay boundary | Normal match/results integration is tracked by G2's match-transition work; no R0.13 blocker remains. RE-190 (R1's own broader "all required framebuffer paths render" bullet) found one further content-bearing mechanism outside R0.13's scope — `sc1PStageClearCopyFramebufToWallpaper` (1P Mode Stage Clear wallpaper) — not yet implemented; every other decomp `framebuf` reference is N64-only VI/memory plumbing with no PSP counterpart |
