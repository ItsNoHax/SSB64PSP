...
| Effects / particles | IN PROGRESS | RE-172 traces all 53 manager `EFDesc` records, corrects file-84's shifted material associations, preserves the 12 direct `Gfx`/`DObjDLLink` constructions the generic graph scan omitted, and verifies all 46 unique display-bearing manager assets in the rebuilt pack (668 triangles). RE-173 exhaustively captures those 46 under PPSSPP software: all 43 authored-visible rest objects have centred render content and the three source-proven rest-invisible objects remain blank as required | Effect `AObjEvent32` playback remains; the separate `LBParticle` bytecode/texture-bank renderer is still not implemented |
