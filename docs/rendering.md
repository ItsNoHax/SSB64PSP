...
| Lighting | 🟢 IN_PROGRESS | `PLAN.md` R0.6: per-vertex lit/literal decision is data-driven via the real `G_MW_LIGHTCOL` signal (RE-103/RE-105); RE-164 retains stage X/Y angles and raw normals; RE-165 now retains and applies LIGHT_1/LIGHT_2 colours, with a GE light scoped to fighter draws and disabled for literal primitives | PPSSPP confirms the v22 pack boots at 60 FPS, but the fighter is too dark: initial inherited ambient state needs source tracing and an original comparison before any correctness claim |
