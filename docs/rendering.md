...
| Lighting | 🟢 IN_PROGRESS | `PLAN.md` R0.6: per-vertex lit/literal decision is data-driven via the real `G_MW_LIGHTCOL` signal (RE-103/RE-105); RE-164 retains stage X/Y angles and raw normals; RE-165 applies the direct and MObj-originated LIGHT_1/LIGHT_2 writes, with a GE light scoped to fighter draws and disabled for literal primitives | PPSSPP confirms the v22 pack boots at 60 FPS and Mario's source ambient removes the black silhouette; original-game visual comparison is still required before any correctness claim |
