...
| Combiner | 🟢 IN_PROGRESS | `PLAN.md` R0.6: general `(A-B)*C+D` two-cycle evaluator (RE-039/RE-043); `combiner_texture_blend` (RE-073/RE-074, device-verified on Link's own model), `combiner_flat_color` (RE-080), shade-scale consumption (RE-106); 97.5%+ of 262,778 combiner-bearing primitives archive-wide recognized in RE-079 | Remaining ~2.5%; the old `prim_color`/`env_color` absence count predates RE-163 resolving every material table and must be remeasured rather than attributed to R0.7 |
| Lighting | 🟢 IN_PROGRESS | `PLAN.md` R0.6: per-vertex lit/literal decision is data-driven via the real `G_MW_LIGHTCOL` signal (RE-103/RE-105); RE-164 retains stage X/Y angles and raw normals; RE-165/166 preserve direct and MObj-originated LIGHT_1/LIGHT_2 writes including zero-valid transitions; RE-167 restores the combiner's per-primitive `PRIMITIVE * SHADE` scale as GE material colour. GE lighting is scoped to fighter draws and disabled for literal primitives | Matched original-ROM/PPSSPP Dream Land Wait comparison restores Mario's red/blue costume semantics; two deterministic PPSSPP captures are byte-identical. Exact cross-renderer pixels are not claimed; physical PSP remains R2 |
