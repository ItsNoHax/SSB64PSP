...
| Effects / particles | IN PROGRESS | RE-172–RE-179 cover all 53 manager `EFDesc` records and all 46 unique display-bearing manager assets, including transform, material, texture and live colour playback. `romtool effects` reports 24/26 material-animation tables replayable; PikachuUnk's table address is fixed, while ImpactWave and MBallThrown are confirmed source-unreachable rather than converter failures. RE-179 maps animated PRIM/ENV/LIGHT_1/LIGHT_2 onto the existing source-classified PSP paths and verifies the frame-4 result across all 26 material-animated manager effects; the manager census contains no animated BLEND track. RE-180 strictly decodes all nine independent `LBParticle` banks: 160 scripts, 65 texture series, 246 image frames and 6,070 used bytecode bytes; every image passes the existing N64 texture decoder. Facing-dependent manager streams plus LBParticle pack/runtime/PSP drawing remain |
