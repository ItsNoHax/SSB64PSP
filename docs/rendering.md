...
| Texture filtering | VERIFYING — reopened (RE-218) | RE-124 measured all 151/151 real `G_MDSFT_TEXTFILT` commands as `G_TF_BILERP`, matching the PSP path's `Linear` filter *mode*; RE-218 found that measurement never compared the N64 RDP's actual 3-point reconstruction formula against PSP's four-tap bilinear, which are different operations sharing a name | `PLAN.md` R2.0/P0a must build a host-side 3-point reference sampler and measure PSP `Linear`'s real error against it on representative textures, then either confirm equivalence or record an `ACCEPTED_DEVIATION` |
| Texture addressing | VERIFYING — reopened (RE-218) | `PLAN.md` R0.5/R0.2: repeat (RE-044/RE-066), mirror (RE-067, pre-baked at pack time), clamp (RE-102, native `sceGuTexWrap(Clamp, ...)` per axis), masks/shifts (RE-044) all measured archive-wide and reproduced for the specific cases each investigated. RE-152 rebases nonzero `G_SETTILESIZE` origins on clamped axes while preserving repeat-axis mask phase, fixing Fox's isolated lower-face primitive in PPSSPP. RE-218 found none of this was checked against a full N64 tile-addressing reference model for coordinates beyond the first mirrored period, `mask == 0`'s own special-case semantics, or PSP's zero-filled power-of-two texture padding vs the N64 logical clamp boundary | `PLAN.md` R2.0/P0b must build that reference model (covering authored UVs and texgen alike) and measure all three questions against real archive content; R2.1/T6/T7 consume it rather than duplicating it; physical PSP validation remains part of R2 |
