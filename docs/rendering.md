...
| Lighting | 🟢 IN_PROGRESS | `PLAN.md` R0.6: per-vertex lit/literal decision is data-driven via the real `G_MW_LIGHTCOL` signal (RE-103/RE-105); RE-164 additionally traces the original per-fighter `ftDisplayLightsDrawReflect` call and retains both stage X/Y angles and raw signed normals in pack version 21 | GE directional-light configuration and a verified safe lit/literal draw split remain to be implemented; no longer treat the prior shared baked direction as a final accepted deviation |
