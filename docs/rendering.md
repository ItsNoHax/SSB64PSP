...
| Texture decode | 🟢 85% | `PLAN.md` R0.3 `COMPLETE`: RGBA16/32, IA4/8/16, I4/8, CI4/8 decoded, unit-tested; 680/706 bound textures packed (RE-163) | 26 unconverted runtime-framebuffer references, each owned by R0.13 |
...
| Runtime MObj display state | IN PROGRESS | The decomp's `gcDrawMObjForDObj` (`refs/ssb-decomp-re/src/sys/objdisplay.c:1150-1410`) emits runtime defaults, texture enable/scale, tile translation/scroll, current/next texture selection, fractional primitive alpha, and dynamic colour/light state; the port currently models static display-list state plus palette/texture animation subsets | End-to-end `MObj` state parity remains open; `MOBJ_FLAG_FRAC` and UV scroll are tracked as focused sub-items in `TODO.md`, while live effect colour consumption is tracked in the Effects / particles row |
| Effects / particles | IN PROGRESS | RE-172 traces all 53 manager `EFDesc` records, corrects file-84's shifted material associations, preserves the 12 direct `Gfx`/`DObjDLLink` constructions the generic graph scan omitted, and verifies all 46 unique display-bearing manager assets in the rebuilt pack (668 triangles). RE-173 exhaustively captures those 46 under PPSSPP software. RE-174 packs and replays all 35 descriptor-selected transform `AObjEvent32` tables (65 nodes) and captures every deterministic frame-4 pose: 35 unique identities, 34 visible, Link Spin Attack correctly material-hidden. RE-175 packs and host-replays the material `AObjEvent32` tables: pack format v24 adds a primitive-level `mat_anim` attachment independent of the texture-only palette cycling RE-089–091 built, `MaterialJoint::track_color` interpolates `Kind::Linear` colour tracks (not just `Kind::Step`), and a new `EffectMaterialAnimator` restarts a spawned effect's own scripts at frame 0. `romtool effects` resolves 17/26 tables and confirms Link Spin Attack's primitive alpha measurably ramps up by frame 4, against the real ROM. RE-176 wires the texture-swap half into `psp/src/meshdraw.rs` (`apply_material` resolves an effective texture through `EffectMaterialAnimator::resolved_texture` before the existing bind path) and PPSSPP-captures all 26 material-animated objects: 24/26 visible at frame 4, the same 2 (NessPKFlash, LinkSpinAttack) correctly rest-invisible. RE-177 fixes 6 of the 9 remaining tables (CommonSpark, DamageFlyMDust, ShockSmall, FalconPunch, FalconKick, YoshiEntryEgg): traced against `refs/ssb-decomp-re/src/sys/objdisplay.c`, their primitives never carry a static pixel `G_SETTIMG` because real hardware supplies it (and re-enables texturing) from inside the same runtime-patched graphics-heap call this converter cannot follow. A new `MeshMaterial::texture_shape` recovers format/size/dims/wrap from the primitive's own static `G_SETTILE`/mask state instead, letting `pack_mesh` convert each sprite variant against it: `romtool effects` now resolves 23/26 tables | The other 3 material tables (ImpactWave, PikachuUnk, MBallThrown) resolve no script/texture for a different reason each (a `matanim.rs` track-decode gap; two node-count/table-alignment mismatches), still needing their own per-file tracing. Live colour-track GE state is resolved and unit-tested but not yet consumed by the renderer. Facing-dependent alternate streams and the separate `LBParticle` bytecode/texture-bank renderer also remain |
...
| Framebuffer effects | COMPLETE | `PLAN.md` R0.13: LB-transition capture/binding are implemented; all 13 files are visually verified (RE-116); RE-146/147 resolve, pack and replay all 11 wipes; RE-148 adds the synchronized capture-before-results lifecycle and PPSSPP-verifies Aeroplane at 60 FPS; RE-149 records the renderer/gameplay boundary | Normal match/results integration is tracked by G2's match-transition work; no renderer blocker remains |
