...
| Framebuffer effects | IN_PROGRESS | `PLAN.md` R0.13: LB-transition one-time capture and binding are implemented; RE-109/111 fixed tile-origin and pillarbox-source errors; all 13 transition files are visually verified on the real device profile (RE-116). RE-146/147 resolve, pack and replay the original 11-entry results-wipe table | No results-state caller yet requests the capture or runs a wipe, so real trigger synchronization remains unverified |
