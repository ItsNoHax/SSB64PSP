...
| Effects / particles | IN PROGRESS | RE-172 traces all 53 manager `EFDesc` records, corrects file-84's shifted material associations, preserves the 12 direct `Gfx`/`DObjDLLink` constructions the generic graph scan omitted, and verifies all 46 unique display-bearing manager assets in the rebuilt pack (668 triangles). RE-173 exhaustively captures those 46 under PPSSPP software. RE-174 packs and replays all 35 descriptor-selected transform `AObjEvent32` tables (65 nodes) and captures every deterministic frame-4 pose: 35 unique identities, 34 visible, Link Spin Attack correctly material-hidden | Effect material `AObjEvent32`, facing-dependent alternate streams, and the separate `LBParticle` bytecode/texture-bank renderer remain |
