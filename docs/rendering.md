...
| Framebuffer effects | IN_PROGRESS | `PLAN.md` R0.13: LB-transition capture/binding are implemented; all 13 files are visually verified (RE-116); RE-146/147 resolve, pack and replay all 11 wipes; RE-148 adds the capture-before-results lifecycle and PPSSPP-verifies Aeroplane at 60 FPS | The normal-game results state does not exist yet to invoke the reusable lifecycle; final trigger synchronization remains open |
