...
**For these seven movement motions, the spare script is `TransN`, with no
exceptions.** The 19 are Kirby,
...
The later grab-motion investigation (RE-331) found another reason for a
spare script: a disabled trailing model part. Packer mapping now checks the
source motion flags as well as the table length.
