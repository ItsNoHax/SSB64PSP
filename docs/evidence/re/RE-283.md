...
Status: IN_PROGRESS — defect catalog confirmed; Fox's wrist-node colour
mechanism now fully explained (real `PRIM * SHADE` math, not a byte/decode
bug), but whether that mechanism is correct vs. real N64 hardware is not
yet settled; other 8 fighters' symptoms still untraced
