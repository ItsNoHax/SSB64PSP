...
itself, not the wrap mode. Captain Falcon's shoulder pad is node-isolated
(node 10, file 332) but not closed — the two leads tested this session (the
small-paletted-texture bypass, and the mat-anim/CLUT-swap path) were both
directly ruled out, and the node's real three-primitive signature doesn't
yet explain the symptom either; do not assume this item is "the same fix"
without re-reading this file's own Captain Falcon section first. Ness,
