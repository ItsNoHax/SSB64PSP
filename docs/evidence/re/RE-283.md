...
**Not yet resolved:** whether `#321d01` is (a) a genuine pipeline bug — e.g.
a `lit`/unlit misclassification of this specific vertex batch (RE-240's/
RE-241's named bug class: a raw signed normal byte reinterpreted as an
unsigned colour component would plausibly produce a dark, saturated,
non-grey triple like this one) that should instead resolve to a light
grey/white matching the adjacent glove and cuff — or (b) an authored dark
"shadow" vertex colour that the original N64 renderer keeps fully hidden
(sleeve/glove geometry overlapping the joint in the original's own posed
skeleton) and this port's own per-node baked transform exposes at a slightly
different angle/scale than the original. Distinguishing these needs either
the raw ROM vertex bytes for node 11's `dl 0x22B8` batch (not yet dumped —
no existing `romtool` command reads raw `G_VTX` colour bytes directly; would
need another small temporary diagnostic) or a live original-N64 reference
capture of Fox's wrist at a comparable angle (this project's `n64-emulator`
Skill, RE-276's own method for Mario). Next step: get one of those two, then
decide fix vs. accepted-deviation.
