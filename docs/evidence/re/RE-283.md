...
(node 25/30, both feet identically) is fully closed too: traced through
`ssb-rom`'s mesh/combiner conversion, `tools/romtool`'s packing, and the PSP
runtime texture bind (all proven clean by live instrumentation), confirmed
as a genuine PSP GE rendering defect for this texture's packed shape on both
PPSSPP and physical hardware, and fixed by bypassing PSP's paletted-texture
transport for this one texture (`Psm8888` instead of `PsmT4`, the same class
of fix RE-267 already used for Donkey Kong) — confirmed removed on PPSSPP
(node-isolated and full-pose) and on physical PSP hardware; a stride/swizzle
hypothesis was tested first and rejected. Yoshi, Captain Falcon, Ness,
Pikachu, Kirby remain untraced. A separate, unrelated dark/purple speckle
pattern on Link's shin cuff was noticed but not investigated this session —
not part of the traced/fixed boot defect.
...
and fixed as the same defect class as the boot, on PPSSPP; physical
confirmation blocked by a USB-permission environment issue, not attempted.**
User directive: trace this item next, after the boot closed.
...
**Physical hardware: not attempted, blocked.** The physical PSP (Slim,
same unit prior follow-ups used) is connected and enumerates over USB
(`lsusb` shows `054c:01c9`), but `usbhostfs_pc` fails to claim the device
("Permission error while opening the USB device") despite the project's own
`50-psplink.rules` udev rule being installed — the live device node shows
mode `0664`/`root:root`, not the rule's `MODE="0666"`, meaning the rule
never actually applied to this device instance (needs `udevadm
control --reload-rules && udevadm trigger`, or a cable replug, either of
which needs interactive `sudo` this session does not have). This is an
environment-access blocker, not a code or evidence gap — flagged rather than
forced. The boot fix's own physical confirmation (thirteenth/fourteenth
follow-ups, same unit) already established that this exact texture-shape
class of GE quirk (small paletted `PsmT4`, 8-byte rows) reproduces
identically on real silicon, not just PPSSPP — this follow-up's fix is the
same mechanism applied to a second instance of that same shape, which lends
reasonable but not equal-strength confidence relative to a direct physical
re-check. Physical confirmation of this specific fix remains a follow-up.

**Confidence (Link shin-cuff speckle: fixed on PPSSPP, high; physical
hardware, not yet confirmed this session — medium-high by analogy to the
boot fix's own already-physically-confirmed mechanism).** Closes RE-283's
Link shin-cuff item on the PPSSPP leg. Yoshi, Captain Falcon, Ness, Pikachu,
Kirby remain untraced. No live original-N64 capture was taken for this item:
