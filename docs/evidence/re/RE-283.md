...
Status: IN_PROGRESS — defect catalog confirmed; Fox's wrist-node colour is
fully explained (node 11's own ROM-authored `SetCombine`/`SetPrimColor`,
converted correctly — not a decode bug, not carried/leaked state); only
open question is whether real N64 hardware draws this same geometry
visibly (needs a live original-N64 capture); other 8 fighters' symptoms
still untraced
