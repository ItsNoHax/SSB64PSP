...
(node 25/30, both feet identically) and its combiner decoded exactly
(`(TEXEL0*SHADE)*ENVIRONMENT`), but the root cause is not yet found — the
static analysis contradicts the observed render (low-medium confidence, see
below). Yoshi, Captain Falcon, Ness, Pikachu, Kirby remain untraced.
...
**Confidence (Link boot defect): low-medium.** High confidence the defect is
real, reproducible, and identical on both feet (isolated-node PPSSPP
captures, pixel-identical hex color read directly from the render). Low
confidence on mechanism: every simple hypothesis (texture, light, vertex
colour, asymmetry, naive env-default) is directly falsified by hand-checked
evidence, and the one surviving combiner shape's own symbolic reduction
contradicts the observed output — meaning either a live material-animation
value or a real conversion-time bug remains to be found by instrumenting
`mesh.rs`'s actual `State` walk directly (not re-deriving it by hand) the
next time this item is picked up. Left open with this as the concrete next
step, per this project's own "measure, then open a follow-up" convention.
