...
Status: IN_PROGRESS — Fox's wrist defect and Samus's chest "black square" are
both fully closed (live N64 capture). Mario/Luigi's missing-button item is
closed on the same "own ROM-authored state, correctly converted" bar
RE-224/P0b-d's addressing work already proved archive-wide (high confidence,
no live-hardware capture taken this follow-up). Samus's thigh speckle does
not reproduce at this camera distance. Link's boot defect is node-isolated
(node 25/30, both feet identically); live instrumentation of the real
`mesh.rs`/`romtool pack` path (not hand algebra) now confirms every
ROM-data input and every line of conversion logic for this primitive is
clean — single-cycle `TEXEL0*SHADE` (not two-cycle as first read),
`env_color` genuinely unset throughout, lighting neutral and ROM-authored,
UVs/tile addressing standard, palette confirmed brown/tan — so root cause is
now known to sit downstream, in pack-time texture serialization or the PSP
runtime texture bind path, not in `ssb-rom`'s mesh/combiner conversion
(medium confidence, see below). Yoshi, Captain Falcon, Ness, Pikachu, Kirby
remain untraced.
...
**Confidence (Link boot defect, revised): medium.** High confidence the
defect is real and reproducible; now also high confidence every ROM-authored
input and every line of `mesh.rs`'s conversion logic for this primitive is
correct — this was directly instrumented and observed live, not inferred.
Medium confidence the remaining surface (pack-time texture serialization,
runtime texture upload) is where the true cause lives, since it is the only
part of the pipeline this session did not yet instrument.
