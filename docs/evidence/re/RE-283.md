...
Status: COMPLETE (software-traced) — Fox's wrist defect, Samus's chest "black square", and
...
and physical PSP hardware. **Ness's shoulder-strap noise is now closed
too** (follow-up session): a raw `dlraw`-style decode of node 4's list again
gave a false lead (three clean 16x32 textures, bypassing them changed
nothing); the authoritative pipeline (`romtool scene --dump-node`) recovered
the real bound texture (an 8x8 CI4 at offset `0xBDE8`, mirror-narrowed to
8x16), and the same `Psm8888` bypass applied to it removes the scrambled
collar patch completely, confirmed on both PPSSPP and physical PSP
hardware. **Pikachu and Kirby are now traced too (follow-up session): neither
reproduces the scrambled-CI4 defect class in its tested default Wait pose.**
Pikachu's "cracks" report traces to a genuine, uncorrupted ROM gradient
decal rendered via correct clamp-overflow addressing (bypass tested, zero
effect — a clean negative result); its "face colour mismatch" is not
present in the current build. Kirby's only reachable body texture (a face
decal) matches its raw ROM data exactly, and its arms/feet carry no texture
at all, so nothing in this defect class could apply; the file's other,
stranger-looking textures all belong to out-of-scope ability-hat graphs or
non-default costumes, not the tested model. All nine originally reported
items are now accounted for.
...
hardware.** File 335, graph `0x26B0` (the `regression_capture_ness` scene).
The user's "right shoulder strap area shows scrambled pixel noise" is a
patch of red/green/black-speckled pixels at the collar, where the striped
shirt sleeve meets the neck — confirmed pixel-level in the committed golden
crop before any tracing began.

*Node isolation.* `romtool scene --file 335 --nodes` finds two near-
identical 27-node graphs (`0x26B0`/`0x4FE8`, front/back-facing or costume
variants — `regression_capture_ness` uses `0x26B0` specifically) plus two
small standalone graphs. Node 8 (the head, `dl 0x1B88`) isolated cleanly at
the correct screen position first, confirming this session's node-isolate
build reused this chain's own established camera framing correctly before
trusting any other candidate. Node 4 (`dl 0x1930`, the right upper-
arm/shoulder segment, child of node 3 in the `3→4→5→6` arm chain) isolated
alone reproduces the exact scrambled patch at the exact collar position the
golden shows — confirmed by a tight crop match, not just "some noise
somewhere".

*A raw `dlraw`-style decode of node 4's list first gave a wrong answer, a
repeat of this chain's own already-documented failure mode.* A temporary
raw display-list decoder (independent of `convert_sequence`, this session's
own instance of the pattern `docs/evidence/re/RE-283.md`'s Falcon section
already named) decoded node 4's list as binding three clean 16x32 CI4
textures (offsets `0xAD00`/`0xAE30`/`0xAB20`, plain shirt-stripe fabric,
each an 8-byte-row `PsmT4` shape too) — but bypassing all three individually
and together, rebuilding the pack each time, produced **zero visible
change** in the isolated capture, byte-identical every time. This is not
what a real fix looks like; per this project's own established practice
(cross-check a raw single-list decode against the authoritative `pack()`/
`plan_draw_order` pipeline before trusting its offsets), the raw decoder was
wrong, not the hypothesis.

*The authoritative pipeline gives the real texture.* Added a temporary
`romtool scene --dump-node <n>` diagnostic printing each primitive's
resolved `MeshMaterial` from the real `convert_sequence` call `pack()`
itself uses (not a from-scratch reimplementation). For graph `0x26B0`'s node
4, this reports exactly one texture bind: an 8x8 CI4 texture at offset
`0xBDE8`, `Clamp`/`Clamp` addressing narrowed by a mirror to a drawn 8x16 —
completely different from the raw decoder's three offsets, and the same
small-paletted packed shape (8-byte-row-or-narrower `PsmT4`) as Link's
boot/shin-cuff, Yoshi's head and Falcon's shoulder. `romtool texdump`
confirms the raw ROM texture at `0xBDE8` is a plain, clean yellow/black
stripe pattern — no purple/red/green corruption present in the source data,
same as every other item in this chain.

*Fix, confirmed.* Bypassing PSP's paletted-texture transport for this one
texture (`Psm8888` instead of `PsmT4`, same fix class as every other closed
RE-283 item) cleared the defect completely in the node-isolated capture —
smooth yellow/purple stripe shading, no residue. Applied permanently
(`tools/romtool/src/main.rs`'s `convert_texture`, new `is_ness_shoulder`
arm, `src.home.id == 335 && data_offset == 0xBDE8`). Rebuilt the full-pose
`regression_capture_ness` capture: diffed against the stale golden, 1,516
differing pixels in a tight bbox `(418,268)-(555,309)` — exactly the collar/
shoulder region, nothing else in the frame moved. Golden refreshed;
`tools/verify-fighter-goldens.sh` re-run for all 12 playable fighters — all
12 pass, Ness against its freshly refreshed golden, the other 11 pixel-
identical to their existing goldens. All scratch instrumentation (the raw
`dlraw`-style decoder, the `--dump-node` diagnostic, and the node-isolate
plumbing in `psp/src/main.rs`/`psp/src/meshdraw.rs`) reverted after use
(`git checkout --`); only the permanent `is_ness_shoulder` arm survives in
`tools/romtool/src/main.rs`.

**Verification (both platforms, same bar every other closed RE-283 item
met):** PPSSPP — node-isolated capture confirms node 4 renders clean with no
scrambled patch, and the full-pose capture matches the refreshed golden
exactly (0 differing pixels on rebuild). `cargo fmt --all --check` and
`cargo test --workspace` (618 passed) clean; plain feature-free `cargo psp
--release` EBOOT hash unchanged (`9b9bfb8f...902ca8c`), confirming the fix
is entirely pack-time (`tools/romtool`), not PSP runtime code — the same
signature every prior `Psm8888`-class RE-283 fix left. Physical — same unit
as every prior RE-283 hardware check (PSP Slim, firmware 6.61, ARK/Infinity,
PSPLink v3.2.1): native `scrshot` of the full-pose `regression_capture_ness`
build shows the identical clean striped shoulder with no scrambled patch, no
exceptions (`exlist` empty, `main_thread` alive throughout).
`tests/golden/r2-ness-fighter.png` refreshed and committed.

**Confidence (Ness shoulder strap): high.** Same bar as every other closed
RE-283 item — node-isolated on both platforms, full-pose golden refreshed
and re-verified against all 12 fighters, root cause positively identified
via the authoritative conversion pipeline (not just "defect disappeared"
from a guess), and the raw-decoder false lead directly demonstrates why this
chain insists on that cross-check rather than trusting a from-scratch raw
decode.

**Pikachu and Kirby traced (follow-up session): neither reproduces the
scrambled-CI4 defect class; both close as "no reproducible defect", not a
fix.** Node-isolation (temporary `romtool scene --dump-node`, reverted
after use, and a temporary `RE283_ONLY_LOCAL_NODE` env-gated draw filter in
`psp/src/main.rs`, also reverted) was applied to both fighters' default
Wait-pose graphs (`FighterRegressionScene`'s graph 341/`0x2650` for Pikachu,
328/`0x1448` for Kirby, both costume 0, matching the committed goldens).

*Pikachu.* Crop-level review of `tests/golden/r2-pikachu-fighter.png` found
no face-colour mismatch (eyes/cheeks pixel-sampled clean: correct black/
white/blue-free iris, correct red cheek hue) — this symptom is not present
in the current build, consistent with RE-281's CI4/`PsmT4` nibble-order fix
having already changed Pikachu's golden (unlike Kirby's, RE-281 explicitly
lists Pikachu among the goldens it *did* change). The reported "cracks" was
found and reproduced: a dark horizontal streak across the collar, present
in both the full-pose golden and a bare node-1 isolation. Traced to an
8-byte-row CI4 gradient decal at file 341 `+0x7350` (16x8, `mirror_s=true`,
`clamp_t=true`), bound identically to nodes 1 and 2 in both of file 341's
graphs. Unlike every other RE-283 item, **the `Psm8888` paletted-transport
bypass (same fix class as Fox/Mario-Luigi/Samus/Link/Yoshi/Falcon/Ness) made
zero pixel difference** when applied to this texture and rebuilt — a clean
null result, not an unexplained one: `romtool texdump` confirms the raw ROM
texel data is itself a smooth, uncorrupted yellow-to-brown gradient (not a
scrambled/speckled source, unlike this chain's other closed items), and the
primitive's own UV span (`uv span 108.9x27.8` = 6.8 S-repeats under mirror,
3.47 T-repeats under **clamp**) predicts exactly this outcome: clamping
past a texture's edge holds that edge texel, so a small vertical gradient
sampled 3.47x past its own height under clamp *should* paint a long solid
band of the gradient's darkest (bottom) row — precisely the streak seen.
This is the project's own established mirror-pre-bake-plus-clamp addressing
(RE-067/RE-102) behaving as designed on genuine, uncorrupted ROM content,
not a PSP GE rendering defect. No fix applied; nothing to bypass.

*Kirby.* `romtool textures --file 328` lists 7 unique bindings, but
node-isolating every textured/geometry node in the captured graph
(`--dump-node` on nodes 2/6/7/20/25) shows the *only* texture reachable
from the base Wait-pose model is a single 32x32 CI4 face decal at
`+0x1CF60` (eyes/cheeks/mouth), bound identically across this file's three
"body" graphs (`0x1448`, `0x2CD0`, `0x19F08`); `romtool texdump`'s raw
decode of it is pixel-identical in content to the rendered face crop —
correct blue/black eyes, red cheeks, black mouth dot, no mismatch. Kirby's
arms and feet (nodes 6/7/20/25) carry **no texture at all** — solid
Gouraud-shaded geometry — so the small highlight/shading variation visible
on the right arm in the golden is ordinary per-vertex lighting, not a
texture artifact, and outside what a paletted-transport bypass could ever
affect (there is no texture there to bypass). The file's other 5 textures
(`+0x1BE30` 16x16, `+0x1D188` 16x8, `+0x1AAF0` 32x32, `+0x1BF50` 32x24, and
a cross-file 32x16 decal borrowed from Yoshi's own file 338 `+0x9EF0`,
which decodes to Yoshi's own red saddle-strap shape) all belong to file
328's other, smaller scene graphs (`0x4230`, `0x49D8`, `0x16AB0`, `0x176D8`,
`0x1D388` — 2 nodes each, consistent with Kirby's per-copy-ability hat
models, not the base body) or to Kirby's four non-default costumes (5 total
per `FIGHTER_COSTUME_COUNTS`); none of them are reachable from the captured
costume-0 Wait pose, so they cannot be the source of a defect in the
current golden regardless of their own correctness. The user's
"weird ear texture" report most plausibly names one of these
out-of-scope ability/costume assets (or the arm's plain shading,
mis-described) rather than anything in the tested body model; a genuine
Kirby-model defect was not found and not reproduced.

**Confidence (Pikachu/Kirby): high that no scrambled-CI4-class defect
exists in either fighter's tested default Wait pose** — every texture
actually reachable from the captured graph was individually node-isolated,
decoded from raw ROM, and (for Pikachu's candidate) bypass-tested with a
clean negative result backed by an independent addressing-math explanation,
not just "looks fine now". **Lower** that either symptom, as originally
reported, referred to exactly what was examined here — costumes other than
0 and Kirby's per-ability hat graphs were not captured or tested, so a real
defect could still exist there; this is future work if a costume-select or
copy-ability scene is ever added to the golden suite, not a standing R2
blocker.

**Costume follow-up (same session): the non-default-costume half of that
gap is now closed too, clean.** A temporary `RE283_COSTUME_OVERRIDE`
env-gated costume index in `psp/src/main.rs`'s fighter-regression draw call
(reverted after use, same throwaway-diagnostic practice as the node
isolation above) rendered all of Kirby's 4 non-default costumes (yellow,
cyan, orange-red, green — `FIGHTER_COSTUME_COUNTS` lists 5 total) and
Pikachu's 3 non-default costumes through `regression_capture_kirby`/
`regression_capture_pikachu`. All 7 captures are clean: correct recolour
(Kirby's cheek accent recolours with its palette, e.g. dark blue on the
cyan costume, matching a real per-costume colour track rather than a stuck
default), no scrambling, no new artifacts, and — for Pikachu — the collar
streak (this session's own finding above) present and unchanged in severity
across every costume, consistent with it being ROM-authored geometry/
addressing rather than a costume-specific palette bug. Only Kirby's
per-copy-ability hat graphs remain untested; costumes themselves are no
longer an open uncertainty for either fighter.

All nine originally reported RE-283 items are now accounted for: seven
fixed with the `Psm8888` paletted-transport bypass (Fox, Mario/Luigi,
Samus, Link boot, Link shin, Yoshi, Captain Falcon) plus Ness (eighth), and
Pikachu/Kirby found to have no reproducible defect in their tested default
model or any of their non-default costumes. This closes RE-283's
per-fighter chase; only Kirby's untested per-copy-ability hat graphs remain,
