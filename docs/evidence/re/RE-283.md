...
**Confidence (Link boot defect, final for this chain): high the defect is
...
behavior for this texture's exact packed shape.** Whether this is
"acceptable" (an inherent PSP GE limitation, needing an `ACCEPTED_DEVIATION`
record) or "fixable" (e.g. by re-packing this texture's shape differently to
avoid whatever GE quirk this triggers) is not yet determined — that is the
next open question, not a re-litigation of source correctness, which is
already closed for every stage this project's own code touches.
