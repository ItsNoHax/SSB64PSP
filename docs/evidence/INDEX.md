...
| RE-283 | Post-RE-281 texture/geometry defects survive across most playable fighters | IN_PROGRESS — Fox, Mario/Luigi, Samus, and Link (boot defect and shin-cuff defect, both fixed by bypassing PSP paletted-texture transport for that one texture, same class of fix as RE-267; shin-cuff fix PPSSPP-confirmed only, physical re-check blocked by a USB-permission environment issue) closed; Yoshi/Falcon/Ness/Pikachu/Kirby untraced. | texture, fighter, geometry, visual-regression, asset-pipeline, hardware |  |
