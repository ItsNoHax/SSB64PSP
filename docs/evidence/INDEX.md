...
| RE-300 | Mario Fireball's `WEAPON_EXTERNAL` translucency: `alpha_blend` seed fixed, CLUT-alpha runtime rendering sti... | PARTIAL — `alpha_blend` classification bug found and fixed (tested); confirmed on real PSP hardware too (not PPSSPP-only); CLUT/palette/format/narrow-texture-addressing and a state-caching bug all ruled out by direct experiment; root cause still open, needs interactive GE-debugger tracing | fighter, rendering, texture, weapon, visual-regression |  |
