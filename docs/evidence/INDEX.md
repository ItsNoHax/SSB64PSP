...
| RE-300 | Mario Fireball's `WEAPON_EXTERNAL` translucency: `alpha_blend` seed fixed, `DrawState` GE-texture-function cache bug found and fixed | COMPLETE — `alpha_blend` classification bug and a `DrawState` GE-texture-function cache bug in `psp-runtime/src/meshdraw.rs` (`None` conflated "unknown" with "known Modulate") both found and fixed; confirmed on PPSSPP (`tests/golden/f1-training-fireball.png`) and real PSP hardware | fighter, rendering, texture, weapon, visual-regression |  |
