...
| Stage animation | 🟢 70% | The 32-bit `AObjEvent32` joint stream is decoded, packed and **played on device: 35 stages, 215 animated nodes at 60 FPS**. All 206 scripts replay on the host and all 206 are still looping after 600 frames, which is the check that catches a one-word desync (RE-050). Verified on device against a control: differencing two frames twelve seconds apart shows motion over the tree canopy with the animation on, and a pixel-identical canopy with it off (RE-051). Note Dream Land carries no joint animation at all — its scenery is moved by game code, not data. Material animation (12 layers) is read but not played; frame 0 matches the baked colours so nothing renders wrong (RE-048) |
