...
| Mesh conversion | 🟢 85% | 47,696 tris, vertex dedup and material merging; 0 failures archive-wide. **Primitive colours are folded into the vertex shade** the way the N64 combiner does (`PRIM * SHADE`), gated on `G_SETCOMBINE` actually naming `PRIMITIVE` — without that gate Mario's white gloves came out green (RE-039). **Only cycle 0 is decoded**: the second cycle, which on several of Mario's parts and Dream Land's layer 2 is `COMBINED * ENV`, is not modelled, and those parts render white (RE-043). Cross-joint vertex sharing resolved for the rest pose (RE-026); no animated skinning yet |
