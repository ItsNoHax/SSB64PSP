...
| PSP mesh drawing | 🟡 90% / VERIFYING | Indexed GE draws, CLUT textures, addressing, animation, alpha gates, runtime lighting and texgen are implemented and exercised in PPSSPP/physical-PSP scenes. RE-226/RE-227 close raw-normal and LookAt-quantization texgen semantics; independent depth writes, submission order, full DrawState cache isolation and the rest of the `R2.1` texgen queue (T1's cross-node gap, T4–T10) remain open. |
