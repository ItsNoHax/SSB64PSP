...
| Texture → PSP conversion | 🟢 96% | **706 bound, 680 packed** (latest `romtool textures`, RE-162). Cross-file pointers resolve through the archive's extern relocations in the display list (RE-037), in a material's sprite table (RE-046), and in scanned lists (RE-047). The rebuilt pack has **329 mip-carrying texture entries**; mirrored-copy pre-baking remains how the PSP's `Repeat`-only wrap reproduces `G_TX_MIRROR` exactly (RE-067). The 26 remaining failures are the loading-break transition's runtime framebuffer photocopy (RE-055/R0.13); RE-162 resolves the former file-86 `MissingPalette`. **Dream Land's canopy discrepancy is partially resolved** — mirror-wrap is fixed and dither is softer (RE-070), but the magnification diagnosis remains open. |
| DL discovery | ✅ COMPLETE | 1,864 lists across 135 files; converter used as validator (RE-017) |
| Mesh conversion | 🟢 85% | 42,417 tris, vertex dedup and material merging; 0 failures archive-wide. **Primitive colours are folded into the vertex shade** the way the N64 combiner does (`PRIM * SHADE`), gated on `G_SETCOMBINE` actually naming `PRIMITIVE` — without that gate Mario's white gloves came out green (RE-039). **Both combiner cycles are evaluated** into a scale on the vertex shade, declining anything that is not one rather than approximating it (RE-043). Dream Land packs every reference it binds and its ground draws textured (RE-044, RE-046). Cross-joint vertex sharing resolved for the rest pose (RE-026); no animated skinning yet. **RDP material state (texture/palette/combiner) threads across a node sequence like real stateful hardware, not reset per list** — previously measured only archive-wide (378/394 textures resolve with inheritance on), now pinned by a direct unit test that was confirmed to fail on the bug it targets before being confirmed to pass (RE-064). No cross-*object* leakage is possible by construction: the pack builder gives every scene graph its own fresh conversion state. **Nonzero N64 clamp windows now lower correctly to zero-origin PSP textures** — clamped axes subtract `G_SETTILESIZE`'s origin while repeat axes retain absolute mask phase; this fixes Fox's geometrically isolated black lower-face primitive (RE-152). **RE-164–166 remove the data-loss basis of the former baked-lighting deviation** — stage X/Y angles, signed normals, and direct/MObj-runtime light colours now survive in pack v23; `G_MW_LIGHTCOL` has an explicit presence bit so an authored black colour and a changed colour under identical flags both reach the GE. Mario's real ambient `(0x4C,0x4C,0x4C)` removes the black silhouette without a brightness heuristic, while original visual equivalence remains open. **The archive-wide geometry-mode default was backwards and is now fixed** — the RDP's per-frame reset (`sSYRdpResetDisplayList`) turns on `G_ZBUFFER`/`G_CULL_BACK`/`G_SHADING_SMOOTH` by default before any object draws, but this project's converter defaulted every node to all-off; `Z_BUFFER` went from 0.17% to 98.3% of packed primitives after seeding from the real default instead (RE-068). **A texture-driven `ENV`→`PRIM` blend with no shade term is now detected, wired up and verified, not just declined** — 91% of the 79 `SetCombine` commands archive-wide reading `ENVIRONMENT` match `(PRIM-ENV)*TEXEL+ENV`, on 28 files including Link/Ness/Pikachu's own base models; maps exactly to the PSP GE's native `TextureEffect::Blend` at zero VRAM cost (RE-073), now consumed on device and confirmed correct on Link's own model with a before/after screenshot, catching a real latent `bind_texture`/texture-function bug along the way (RE-074). RE-163 resolves every discovered graph's material table (127 graphs / 467 matching nodes); the former primitive/environment-colour absence count predates that result and is queued for a fresh R0.6 census rather than still being blamed on missing tables. |
...
| PSP mesh drawing | 🟢 88% | Indexed GE draws, CLUT textures, measured repeat/mask/mirror/clamp addressing, baked shading, per-node matrices, and per-primitive depth. Pack version 23 preserves normals and source LIGHT_1/LIGHT_2 values, including an authored zero and a changed value under identical material flags; the GE now configures the original stage-angle directional light only around fighter draws and enables it only for `LIT` primitives (RE-165/166). Original-game visual comparison remains the active correctness investigation. Alpha-tested cutouts render correctly (RE-069); RE-129/130 enable real source-alpha blending for the two classified single-cycle alpha formulas while declining the measured rare/two-cycle long tail. RE-144 restores the original billboard X/Y/X scale rule and adds isolated per-node inspection. |
...
436 host tests passing across `ssb-rom` (282), `ssb-engine` (36) and
`ssb-game` (118). Reproduce with `cargo test --workspace`.
