...
| PSP GU backend | 🟡 89% / VERIFYING | Init/frame lifecycle, matrices, indexed textured mesh draws, CI4/CI8 CLUT upload, mip-level upload, filtering, repeat/clamp addressing, alpha test/blend, depth/culling, billboard transforms, and runtime fighter lighting are implemented in `psp/src/gu.rs` and `psp/src/meshdraw.rs`. Independent depth compare/write state is wired (RE-251, `R2.2`/C3, `COMPLETE`); the systematic raw-GU-mutation inventory found one live cache bypass (already fixed, RE-118) and hardened five other sites with `DrawState::invalidate_all()` (RE-254, `R2.2`/C5, `COMPLETE`) |
