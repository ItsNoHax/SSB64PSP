...
| Mesh conversion | 🟢 85% | 47,696 tris, vertex dedup and material merging; 0 failures archive-wide. **Primitive colours are folded into the vertex shade** the way the N64 combiner does (`PRIM * SHADE`), gated on `G_SETCOMBINE` actually naming `PRIMITIVE` — without that gate Mario's white gloves came out green (RE-039). **Both combiner cycles are evaluated** into a scale on the vertex shade, declining anything that is not one rather than approximating it (RE-043). The surfaces that still render white are texture-conversion failures, not colour ones: Dream Land packs 16 of the 19 textures it binds. Cross-joint vertex sharing resolved for the rest pose (RE-026); no animated skinning yet |
