...
| Animation | 93% | Figatree scripts decode to per-joint transforms and are packed; 189 movement-animation lengths plus Mario's 46-frame Fireball, 40-frame Super Jump Punch (including its hidden TransN entry), and 87/83-frame Tornado replay exactly against the ROM; skeleton ticks at 60 FPS on device and now supplies TransN delta/pitch to portable gameplay; all 538 sparse fighter/slot entries replay correctly | RE-036, RE-038, RE-171, RE-299 | `translate_scales` is not required by Mario's up-B; viewer camera frames on rest bounds only | M3 |
...
| Items / weapons | 10% | Mario Fireball is the first match-owned spawned weapon: a fixed-capacity portable pool consumes the frame-16 joint-16 spawn request, applies its sourced launch/lifetime/gravity/map rebound/hitbox values, excludes its owner, routes shield/damage through the shared hit path, and renders its packed source geometry. Its `WEAPON_EXTERNAL` translucency material's `alpha_blend` classification bug is fixed (RE-300) | RE-300 | No general item system; weapon collision is currently Fireball-specific. Fireball's on-device transparency is still broken: it renders as an opaque card, not a translucent flame — root cause narrowed to the PSP GE/PPSSPP CLUT-alpha path, not this project's own state logic (RE-300) | G1 |
...
untouched for every fighter other than Mario: specials; and for every fighter,
grabs/throws (needs
