...
| Texture → PSP conversion | 🟢 96% | **692 bound, 663 packed**, 51.7% VRAM saved (latest `romtool textures`, RE-155). Cross-file pointers resolve through the archive's extern relocations in the display list (RE-037), in a material's sprite table (RE-046), and in scanned lists (RE-047). The rebuilt pack has **314 mip-carrying texture entries**; mirrored-copy pre-baking remains how the PSP's `Repeat`-only wrap reproduces `G_TX_MIRROR` exactly (RE-067). Texture VRAM is **1267.0 KiB, 1.8x the ~700 KiB all-at-once budget**, so streaming remains required. 29 failures are understood: 26 are the loading-break transition's runtime framebuffer photocopy (RE-055/R0.13); the remaining three `MissingPalette` cases are two in file 114 and one in file 86, still owned by R0.7 material-table recovery. **Dream Land's canopy discrepancy is partially resolved** — mirror-wrap is fixed and dither is softer (RE-070), but the magnification diagnosis remains open. |
...
| Scene graph (DObj) | 🟢 85% | All 363 `DObjDesc` arrays recovered and validated against the decomp (RE-023); world transforms baked into the pack. Three union members of `DObj`'s display-list field resolved, and node lists converted in draw order through one shared vertex cache — zero conversion failures archive-wide (RE-025, RE-026). `MObj` material chains recovered for **109 graphs (18 still unpaired)** via `FTCommonPart`, `MPGroundDesc`, `EFDesc` and source-confirmed call-sequence records, including file 85's two static effects and all three source-parallel Bonus2 platforms (RE-155); 436 paired nodes have zero chain/demand mismatches. `GObj` layer and animation still absent |
