...
| Texture → PSP conversion | 🟢 96% | **707 bound, 681 packed** (latest `romtool textures`, RE-172). Cross-file pointers resolve through archive relocations (RE-037/046/047); 329 entries carry PSP mip levels and mirror-copy pre-baking reproduces `G_TX_MIRROR` (RE-067). The 26 remaining non-ROM textures are implemented runtime framebuffer captures (R0.13). RE-201's direct PSP Slim framebuffer capture verifies Dream Land canopy/filter/coordinate behaviour on hardware; R0.5 is `COMPLETE`. |
...
| Asset pack format | ✅ COMPLETE | Zero-copy, 16-byte aligned, little-endian; writer + reader unit-tested, 3,341 meshes and 3,148 scene-graph nodes round-trip. Current pack version 26 includes animation tables, material-animation attachments, lighting state, each node's local rest transform, LBParticle bank/script/texture-series tables, and alpha-compare reference state. |
| PSP asset loading | ✅ COMPLETE | Current 8,283,696-byte pack (version 26, including all LBParticle source data and RE-195 alpha-compare state) loads aligned and cache-flushed; PPSSPP smoke-tested, with earlier smaller packs verified on physical hardware |
| PSP mesh drawing | 🟢 90% | Indexed GE draws, CLUT textures, measured repeat/mask/mirror/clamp addressing, baked shading, per-node matrices, per-primitive depth, manager-effect texture/colour animation, and the disjoint `G_AC_THRESHOLD` alpha gate. Pack version 27 retains version 25's LBParticle and material-animation data, version 26's alpha-compare reference state (RE-195), and adds the `gSPTexture` scale and render-tile origin a `G_TEXTURE_GEN` primitive needs (RE-214). Texture-coordinate generation is separated from texture binding: ordinary `G_TEXTURE_GEN` is produced by the GE's texture-matrix generator from the normalised vertex normal against the camera's world right/up basis, the same basis `syMatrixLookAtReflectF` writes into the RSP's look-at, with the source scale and tile origin carried in the matrix. An archive-wide census (`romtool texgen`) proves the state may stay primitive-level: zero texgen triangles load a vertex under a different mode or scale than they draw under. Verified in PPSSPP and on physical PSP at two model rotations; no original-N64 comparison exists yet. `G_TEXTURE_GEN_LINEAR` (RE-215) is now exact: its `acos` curve is generated per vertex on the CPU, cross-checked against two independent reference implementations, and drawn through the authored-UV pipeline rather than a second GE mode — verified in PPSSPP and on physical PSP against the graph that actually carries it (`regression_capture_scene13`; RE-215 found scenes 11/12 do not, despite an earlier RE-214 note). Both RDP alpha gates now share the GE's one alpha-test unit (`pack::alpha_gate`), overlap included. Mesh textures bind level zero only at constant LOD, matching `G_TL_TILE` (RE-213, revalidated by RE-214). RE-179 applies live PRIM/ENV/LIGHT_1/LIGHT_2 state through source-classified GE colour paths without mutating shared pack vertices. The GE configures the original stage-angle directional light only around fighter draws and enables it only for `LIT` primitives (RE-165/166). RE-167's matched original comparison found and fixed the last observed combiner boundary: runtime lighting now applies the retained `PRIMITIVE * SHADE` scale as GE material colour, restoring Mario's source red/blue costume semantics. Exact cross-renderer pixels and physical PSP remain open. Alpha-tested cutouts render correctly (RE-069); RE-129/130 enable real source-alpha blending for the two classified single-cycle alpha formulas while declining the measured rare/two-cycle long tail. RE-144 restores the original billboard X/Y/X scale rule and adds isolated per-node inspection. |
