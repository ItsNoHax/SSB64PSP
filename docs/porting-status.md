...
| Texture → PSP conversion | 🟢 96% | **706 bound, 680 packed** (latest `romtool textures`, RE-162). Cross-file pointers resolve through archive relocations (RE-037/046/047); 329 entries carry PSP mip levels and mirror-copy pre-baking reproduces `G_TX_MIRROR` (RE-067). The 26 remaining non-ROM textures are implemented runtime framebuffer captures (R0.13). Dream Land's canopy mirror, filter, coordinate, LOD and blur hypotheses have been measured; R0.5 is `VERIFYING` on physical PSP rather than open to further PPSSPP tuning. |
...
| Effects / particles | 🟡 62% | RE-172 inventories the original manager's 53 `EFDesc` records: three are controller-only, four pairs share assets, and all 46 unique display-bearing DObj/direct-list effects survive the pack (`romtool effects`: 46/46, 668 triangles). RE-173 captures every rest identity. RE-174 packs and host-replays all 35 descriptor-selected transform `AObjEvent32` tables (65 nodes), then PPSSPP-captures all 35 frame-4 poses with unique HUDs: 34 visible and Link Spin Attack correctly hidden pending its material alpha stream. RE-175 packs and host-replays the material `AObjEvent32` tables (pack format v24 adds `PrimDesc.mat_anim`; `MaterialJoint::track_color` now handles `Kind::Linear` colour ramps, not just `Kind::Step`): `romtool effects` resolves 17/26 tables and confirms Link Spin Attack's primitive alpha measurably ramps up by frame 4 against the real ROM. RE-176 wires the texture-swap half of `PrimDesc.mat_anim` consumption into `psp/src/meshdraw.rs` and PPSSPP-captures all 26 material-animated objects: 24/26 visible at frame 4, the same 2 (NessPKFlash, LinkSpinAttack) correctly rest-invisible. RE-177 traces the remaining 9 unresolved tables against the ROM and fixes 6 (`MeshMaterial::texture_shape` recovers a sprite's format/dims/wrap from its static `G_SETTILE`/mask state when, as for these 6, real hardware only ever supplies the pixel address at runtime): `romtool effects` now resolves 23/26 tables. | The other 3 material tables (ImpactWave, PikachuUnk, MBallThrown) remain: a `matanim.rs` track-decode gap and two node-count/table-alignment mismatches, each needing its own per-file tracing. Live colour-track GE state (`EffectColors`) is resolved and unit-tested but not yet consumed by the renderer. Facing-dependent alternate streams remain. The independent `LBParticle` script/texture-bank runtime used by dust, flame, sparkle, hit and stage effects is not implemented. |
