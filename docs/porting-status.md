...
| Texture → PSP conversion | 🟢 97% / COMPLETE for measured renderer scope | ROM-backed conversion, mirror/clamp/origin lowering, palette banks and texgen's R2.1/T1–T10 gate are complete (RE-201, RE-219–239). N64 three-point filtering remains a measured PSP fixed-function deviation. Current pack format is version 28. |
...
| Asset pack format | ✅ COMPLETE | Zero-copy, 16-byte aligned, little-endian; writer + reader unit-tested. Current pack version 28 includes animation tables, material-animation attachments, lighting state, local rest transforms, LBParticle data, alpha-compare state, texgen scale/origin state and independent depth compare/write/mode state. |
| PSP asset loading | ✅ COMPLETE on PSP-2000/3000 | Current version-28 pack loads aligned and cache-flushed. RE-260 physically confirms the 26,254,608-byte pack with `MEMSIZE=1` on PSP Slim; PSP-1000's 32 MiB limit remains a separate R2 compatibility requirement. |
