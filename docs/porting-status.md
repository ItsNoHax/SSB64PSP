...
| Base moveset (Mario) | 25% | `AttackDash`/`AttackS3Hi`/`AttackS3`/`AttackS3Lw`/`AttackHi3`/`AttackLw3` (ground) plus `AttackAirN`/`F`/`B`/`Hi`/`Lw` (aerials) ported field-for-field from `dMarioMainMotion_*` (`relocData/202_MarioMainMotion.c`), plus `Attack11`'s second hitbox (was a documented gap, now both real). Hit resolution generalized to a per-`(FighterKind, Status)` `MoveData` table (`crate::attack::move_data`) carrying every hitbox and its own active-frame window, instead of the old Attack11-only special case — `apply_hit_from` now works for any ported status/fighter pair, including a hitbox's `ox` mirrored by facing (closing part of the old "no joint attachment" gap: `oy`/`oz` still are not), multiple simultaneous hitboxes (up to 3, on neutral aerial), weakening hitbox phases (dash attack, all 4 non-neutral aerials), and a genuinely *pulsing* hitbox (down aerial: on 2 frames, off 1, eight times — modelled as 16 real disjoint windows, not one merged range, so landing in a real gap still dodges it). Aerial landing is real too: landing mid-move takes a dedicated `LandingAirX` status where the fighter has one, or `LandingAirNull` scaled by the real extracted `flag1` percentage where they don't (Mario's neutral aerial) — wired into `Fighter::tick_air` via `crate::status::set_landing_or_landing_air`. Entry conditions (stick-angle/magnitude gates, the tilt-vs-jab and attack-vs-second-jump dispatch priorities, dash-attack's `A`-tap from `Dash`/`Run`) are real, transcribed from the matching `ftcommonattack*.c` `CheckInterruptCommon` functions; angle tests are reframed as `tan()` slope comparisons since `ssb_engine::math` has no `atan2`. Fixed a real pre-existing gap alongside the ground moves: `walk_interrupt` was missing `Attack1`/the tilts entirely even though the decomp macro has them, so attacking out of a walk did nothing | — | Smashes (need a charge-mechanic subsystem not built yet), specials, the `Attack12`/`Attack13`/`Attack100` jab-combo extension, grabs/throws; no other fighter has any moveset data yet; `DTilt`'s repeated-tap extension not ported (always exits to `SquatWait` after one hit); `LandingAirF`/`Hi`/`B`/`Lw` collapse to one tick (no extracted animation length, same class of gap as `GuardOn`); the real `SKIPLANDING_VEL_Y_MAX` auto-cancel branch and the "landed after the move's own lag window already closed" branch are both dropped in favour of "landing mid-move always takes the lag status", since the real `SetFlag1`/`SetFlag1(0)` window brackets nearly all of every Mario aerial ported so far | P2 |
