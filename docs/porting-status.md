...
| Items / weapons | 5% | Mario Fireball is the first match-owned spawned weapon: a fixed-capacity portable pool consumes the frame-16 request, applies its sourced launch/lifetime/gravity/floor rebound/hitbox values, excludes its owner, and routes shield/damage through the shared hit path | — | No general item system; no weapon rendering or runtime joint-16 spawn position; shared collision currently supplies floors only, so wall/ceiling rebounds await map-collision work | G1 |
