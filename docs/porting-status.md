...
| **M2 — Resource pipeline** | ✅ COMPLETE | Archive + VPK0 verified; 2722 meshes (47,696 triangles, 0 conversion failures) and 615 textures packed into a 3.0 MB runtime pack that round-trips |
...
| N64 texture decode | 🟢 85% | RGBA16/32, IA4/8/16, I4/8, CI4/8 decoded and unit-tested; 615 real ROM textures decode and ship in the pack |
| Texture → PSP conversion | 🟢 85% | **732 bound, 615 packed**, 72.8% VRAM saved. **Cross-file texture pointers resolve** through the archive's extern relocations, both in the display list (RE-037) and in a material's own sprite table (RE-046) — which is what Dream Land's ground draws by, now the stage's basket-weave underside instead of white. 117 references still fail: 75 are a `G_SETTIMG(0)` nothing overwrites (71 scene graphs the original pairs to a material table in *code*, not in data, so the pairing is not in the ROM — RE-046), 29 paletted without a recorded TLUT, 26 segmented, 16 missing a palette at decode. Those totals rose from 83/54 when the report stopped collapsing every unresolved reference in a file into one entry; they were always failing. Round-trip tested through swizzle+CLUT; verified on device (RE-022). **At 635 KiB the full set fits texture VRAM all at once**: it had been measured at 1.1 MiB because wrapping textures were converted at the size they were *drawn* rather than the size they are (RE-044), so the streaming requirement was an artefact |
| DL discovery | ✅ COMPLETE | 1,864 lists across 135 files; converter used as validator (RE-017) |
| Mesh conversion | 🟢 85% | 47,696 tris, vertex dedup and material merging; 0 failures archive-wide. **Primitive colours are folded into the vertex shade** the way the N64 combiner does (`PRIM * SHADE`), gated on `G_SETCOMBINE` actually naming `PRIMITIVE` — without that gate Mario's white gloves came out green (RE-039). **Both combiner cycles are evaluated** into a scale on the vertex shade, declining anything that is not one rather than approximating it (RE-043). Dream Land now packs 19 of the 20 references it binds and its ground draws textured (RE-044, RE-046). Cross-joint vertex sharing resolved for the rest pose (RE-026); no animated skinning yet |
