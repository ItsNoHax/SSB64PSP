...
| Base moveset (Mario) | 35% | Everything in the *ground and aerial, no-charge, no-combo* part of the moveset: jab, dash attack, all three forward-tilt angles, up/down tilt, all five forward-smash angles, up/down smash, and all five aerials — ported field-for-field from `dMarioMainMotion_*` (`relocData/202_MarioMainMotion.c`). Discovered while researching smashes: **SSB64 smashes have no charge mechanic at all** (that is a Melee addition) — a smash is just a hard flick (`|stick| >= 56` inside a short tap window, `ftCommonAttackS4CheckInterruptCommon`) versus a tilt's plain magnitude push, so smashes turned out to be the same shape as tilts, not a new subsystem. Hit resolution is on a per-`(FighterKind, Status)` `MoveData` table (`crate::attack::move_data`) carrying every hitbox and its own active-frame window — `apply_hit_from` works for any ported status/fighter pair, including a hitbox's `ox` mirrored by facing, multiple simultaneous hitboxes (up to 3, neutral aerial), weakening hitbox phases (dash attack, 4 of 5 aerials), and a genuinely *pulsing* hitbox (down aerial: on 2 frames, off 1, eight times — 16 real disjoint windows, not one merged range). Aerial landing is real: a dedicated `LandingAirX` status where Mario has one, or `LandingAirNull` scaled by the real extracted `flag1` percentage where he doesn't (neutral aerial) — `Fighter::tick_air` → `crate::status::set_landing_or_landing_air`. Entry conditions (stick-angle/magnitude/tap-window gates, the smash-vs-tilt-vs-jab and attack-vs-second-jump dispatch priorities) are real, transcribed from the matching `ftcommonattack*.c` `CheckInterruptCommon` functions; angle tests are reframed as `tan()` slope comparisons since `ssb_engine::math` has no `atan2`. Fixed a real pre-existing gap alongside the ground moves: `walk_interrupt` was missing `Attack1`/the smashes/the tilts entirely even though the decomp macro has them, so attacking out of a walk did nothing | — | Specials, the `Attack12`/`Attack13`/`Attack100` jab-combo extension, grabs/throws; no other fighter has any moveset data yet; `DTilt`'s repeated-tap extension not ported (always exits to `SquatWait` after one hit); the narrow `anim_frame <= 5` pivot-smash-out-of-dash-startup window is not ported (only the main `<= 20` dash-attack window is); `LandingAirF`/`Hi`/`B`/`Lw` collapse to one tick (no extracted animation length, same class of gap as `GuardOn`); the real `SKIPLANDING_VEL_Y_MAX` auto-cancel branch and the "landed after the move's own lag window already closed" branch are both dropped in favour of "landing mid-move always takes the lag status", since the real `SetFlag1`/`SetFlag1(0)` window brackets nearly all of every Mario aerial ported so far | P2 |
