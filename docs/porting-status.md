...
| Texture → PSP conversion | COMPLETE for measured renderer scope | Mirror/clamp/origin/signed-coordinate lowering, palette banks, texgen T1–T10, PSP sample-centre lowering, and selective build-time 3-point compensation (material-aware on alpha; CI4/CI8 use filter-aware index coordinate descent; animated CI shares one index field optimized over reachable `PaletteID` states; direct colour has GE-exact integer refinement and bounded critical-UV training plus independent validation, RE-309/310; texgen trains on real-normal pose coverage inside a pose-independent S10.5 bound, RE-311; `G_MDSFT_TEXTLUT` tracked per task list, measured stale short-TLUT slots, and every source format compensated, RE-313). RE-312's regenerated report additionally validates seven per-primitive UV phases and four denser-training immutable variants. Pack format v31 | RE-201, RE-219–239, RE-262–269, RE-301, RE-304–313 | Exact 3-point equality remains impossible with fixed-function bilinear; measured admission gates retain source indices where joint optimization is insufficient | R0.5, R2.0 |
...
| Asset pack format | COMPLETE | Zero-copy, 16-byte aligned, little-endian; writer + reader unit-tested. v31 adds per-primitive S10.5 phase to v30's animation, per-MObj UV rest state, lighting, rest transforms, LBParticle, alpha-compare, texgen, independent depth state and signed-clamp UV marker | RE-301, RE-312 | — | M2 |
| PSP asset loading | COMPLETE on PSP-2000/3000/Slim | Current v31 pack loads in PPSSPP; the aligned/cache-flushed v29 pack (26,254,608 B) and `MEMSIZE=1` were physically proven | RE-256, RE-260, RE-301, RE-312 | PSP-1000's 32 MiB compatibility unresolved (can't use `MEMSIZE=1`); v31 has no physical PSP capture | R2 |
| PSP mesh drawing | VERIFYING (90%) | Indexed GE draws, CLUT textures, addressing, palette/frame/UV material animation, alpha gates, runtime lighting and texgen implemented; deterministic Dream Land, Meta Crystal, and non-zero Link-costume PPSSPP captures pass | RE-226–239, RE-251–254, RE-261, RE-270, RE-271, RE-301 | `SetLFrac` remains an explicit RDP two-tile limitation | R2.1, R2.2, R2 |
...
| Input mapping | 80% | Nub scaling and the player-facing PSP→N64 table are unit-tested. `psp-game` selects its own raw layout at the backend boundary: nub→stick, D-pad→independent C-buttons, X/Square→A/B, L/R→Z/R, Circle→L; Triangle/Select unbound. C-button jump reaches the existing source input path and menu navigation reads the mapped N64 stick. The asset viewer keeps its legacy layout. | RE-008, RE-295 | Deadzone unresolved; taunt/camera C-button functions still unconfirmed | TODO (RE-008, RE-009) |
...
| Fighter shadows | COMPLETE for Training/runtime scope | Source `ftShadowProcDisplay` strip: file-84 I4 texture, `shadow_size` half-width, floor/polyline clipping and slope contours, airborne floor projection, source alpha/depth/blend/cull state, lifecycle hiding and independent multi-fighter draws | RE-302 | Team-shadow colour mode and moving-map collision groups await the match/stage runtime | — |
...
| Physics | 62% | 16+ functions ported with original addresses cited, driven every tick against real per-character constants (all 27 fighters) extracted from ROM and verified field-by-field against decomp. Fixed a real pre-existing gap: `ftPhysicsCheckSetFastFall` (`crate::status::check_set_fast_fall`) was never called from anywhere — `Fighter::tick_air` read `physics.is_fastfall` but nothing ever set it from a downward stick input, so fastfalling silently did nothing for every airborne status. Now wired into every airborne tick, same as the original calls it from every airborne status's own `proc_physics` | RE-032 | — | M3 |
| Fighter state | 68% | The full `FTCommonStatus` ordinal table (0..=219, `ft/ftcommon/ftcommonstatus.h`) is now in `Status`, matching the decomp's own numbering exactly. Movement status machine (Wait/walks/Dash/Run/Turn/Jump/Fall/Squat/Landing/Pass) unchanged. The Damage/hitstun family (`DamageHi/N/Lw1-3`, `DamageAir1-3`, `DamageFlyN`/`FlyTop`, `DamageFall`) now has real entry/exit: a landed hit picks its status from `ftCommonDamageGetDamageLevel`'s hitstun tiers and the defender's grounded/airborne situation, and hitstun running out returns to `Wait` or `DamageFall` (`ftCommonDamageCommonProcInterrupt`/`...AirCommonProcInterrupt`). Mario's neutral/up/down-B now use sourced extended statuses; Fireball preserves its ground/air map transitions and one-shot frame-16 weapon event, while Tornado preserves its grounded-to-air entry, frame-43 one-way rise gate, and status-specific physics | RE-033, RE-035, RE-294, RE-295, RE-299 | Every status past the wired movement/Damage/base-moveset subset exists as an ordinal only, with no callback behaviour (specials, grabs, smashes, aerials besides what's listed below — `P2` scope). Within Damage: no hit-location Hi/Lw index (every hit reads as "N"/middle), no `DamageFlyRoll` (needs a shared RNG source), and the ground-hit-still-launches-airborne angle branch is dropped (`Status::is_grounded`'s docs) | M3, G0 |
| Base moveset (Mario) | 65% | Mario's jab (full `Attack11`→`Attack12`→`Attack13` chain), dash attack, all forward-tilt/-smash angles, up/down tilt, up/down smash, all five aerials, and every B-special are ported from `dMarioMainMotion_*` (`relocData/202_MarioMainMotion.c`). `SpecialN` uses its distinct 46-frame poses, exact neutral-B turn gate, frame-16 spawn event, and a sourced live Fireball; `SpecialHi` uses real TransN motion; Tornado uses its distinct 87/83-frame poses, thirteen pulses, finisher, B-tap rise, and clamp-decay curve. Training's shared per-target `HitRecord` now follows source `ClearAttackCollAll` generations, so Super Jump Punch's adjacent loop windows re-hit correctly while ordinary sourspot replacements do not; Tornado's separated pulses use the same bridge. A deterministic B+up PPSSPP Training smoke exercises the rendered runtime path. Hit resolution is a per-`(FighterKind, Status)` `MoveData` table carrying every fighter hitbox and active-frame window | RE-299 | `Attack100` (rapid-jab loop, excludes Mario); grabs/throws; `DTilt`'s repeated-tap extension; `LandingAirF`/`Hi`/`B`/`Lw` collapsing to one tick; the real aerial auto-cancel branch. Fireball's visible joint-16 spawn/render path and wall/ceiling rebounds wait on shared runtime/map work | P2 |
| Base moveset (Fox) | 90% | All ground/aerial normal attacks, five-angle forward tilt, rapid jab, Blaster, Fire Fox, and Reflector are source ports. The portable weapon pool handles Blaster flight/hits and Fireball/Blaster reflection; the PSP runtime renders the packed Blaster list and sourced Reflector hierarchy. All 42 Fox move clips are packed and selected by status, and a deterministic PPSSPP Training capture exercises neutral-B end to end | RE-303 | Grabs/throws and generic aerial auto-cancel gaps remain. Fire Fox wall/ceiling response waits on the shared fighter wall/ceiling collision solver; the Reflector hierarchy currently uses its rest transform rather than its four effect animation phases | P2 |
| Recovery (`FallSpecial`) | 25% | `ftCommonFallSpecialSetStatus`/`ftCommonLandingFallSpecialSetStatus` ported (`crate::status::set_fall_special`/`set_landing_fall_special`, `FallSpecialState`) — the shared "helpless fall after a recovery move's launch phase" status every fighter's up-special (and some others) enters. Mario `SpecialHi` now proves the path end-to-end: its 40-frame TransN launch hands off with source `.6` drift, forced landing recovery, `.28` landing lag, and spent jumps | RE-299 | `is_allow_pass`'s drop-through-platform nuance during the fall, and cliff-catch-while-falling (no status auto-catches a ledge yet — a pre-existing gap, not new here), are also not ported | P2 |
...
| KO/death/respawn | 45% | Blast-zone crossing (`crate::status::check_dead`, `ftCommonDeadCheckInterruptCommon`) enters `DeadDown`/`DeadLeftRight`/`DeadUpStar` and decrements a stock immediately, matching the original's bottom→right→left→top priority. After the real `FTCOMMON_DEAD_WAIT`/`DEADUP_WAIT`-derived timer, the caller-invoked `try_rebirth` (needs the stage's respawn point, which `update` has no way to receive) enters `Sleep` if stocks are exhausted or the `RebirthDown`→`RebirthStand`→`RebirthWait` sequence otherwise — real total duration (390 frames) preserved, ending in `Fall` with a real 120-frame invincibility window (`Fighter::invincible_frames`) that both `apply_hit_from` and `apply_shield_hit` now respect. Damage resets to 0 on respawn | — | No 1-in-6 `DeadUpFall` branch (needs an RNG source, same gap as `DamageFlyRoll`); the halo drop/flight/colour-fade visuals, camera-mode switches, and 1P-mode/team-respawn branches are not ported (rendering/scene-manager scope, module docs); no multi-respawn halo-position offsetting; `GuardOn`'s Yoshi-only side cases and Boss/`is_ignore_dead` exemptions not ported | P2 |
| Collision | 65% | Geometry extracted/packed for all 41 stages; swept + projected floor solvers agree on 158/158 spawn tests; Mario Fireball's authored diamond collider now sweeps typed floors, ceilings, and directional walls for its source rebound behaviour | RE-030, RE-031 | Fighter map collision remains floor-only; moving groups tested at rest only | M3 |
| Animation | 95% | Figatree scripts decode to per-joint transforms and are packed; Mario's six special slots and Fox's 42 normal/special slots replay exactly against the ROM; skeleton ticks at 60 FPS on device and supplies runtime joint attachments and TransN motion to portable gameplay | RE-036, RE-038, RE-171, RE-299 | Effect phase animation is not yet connected to Fox's Reflector hierarchy; viewer camera frames on rest bounds only | M3 |
...
| Items / weapons | 18% | The fixed-capacity match pool owns Mario Fireballs and Fox Blaster shots, with sourced spawn attachments, motion, collision, damage, reflection ownership/scaling, and packed source rendering. Fireball transparency is fixed and verified on PPSSPP and real PSP hardware (RE-300) | RE-300 | No general item system; only these two fighter weapons are ported | G1 |
...
**Per-fighter combat progress: Mario and Fox have their normal attacks and
B-specials; the other 10 fighters are at 0%.** `PLAN.md`'s combat-systems and
...
tested through Mario and Fox. Fighter *models*, *animation* and *movement
physics* are implemented and tracked above. Grabs/throws remain deferred
because they need per-character throw motion data and gameplay-facing joint
attachment. Rendering fidelity/performance (`P5`) runs in
