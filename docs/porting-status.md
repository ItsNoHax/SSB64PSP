...
| Texture → PSP conversion | 🟢 85% | 664 bound, **581 packed**, 72.7% VRAM saved. **Cross-file texture pointers now resolve** through the archive's extern relocations, which is what stages reach their texels by: Dream Land went from 1 texture to 18 and renders textured on device (RE-037, `docs/images/m4-stage-textured.png`). 83 references still fail — 54 null pointers nothing names, 28 paletted without a recorded TLUT, 16 missing a palette at decode, 13 segmented. Round-trip tested through swizzle+CLUT; verified on device (RE-022). **At 607 KiB the full set fits texture VRAM all at once**: it had been measured at 1.1 MiB because wrapping textures were converted at the size they were *drawn* rather than the size they are (RE-044), so the streaming requirement was an artefact |
