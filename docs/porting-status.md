...
| Effects / particles | 🟡 35% | RE-172 inventories the original manager's 53 `EFDesc` records: three are controller-only, four pairs share assets, and all 46 unique display-bearing DObj/direct-list effects now survive the pack (`romtool effects`: 46/46, 668 triangles). File-84's shifted Catch Swirl/Reflect Break/Dead Explode/Ness PK Flash material associations are corrected from current decomp and ROM evidence. | Effect `AObjEvent32` playback and exhaustive PPSSPP capture remain. The independent `LBParticle` script/texture-bank runtime used by dust, flame, sparkle, hit and stage effects is not implemented. |
