...
| Texture → PSP conversion | 🟢 90% | **647 bound, 617 packed**, 74% VRAM saved. Cross-file pointers resolve through the archive's extern relocations in the display list (RE-037), in a material's sprite table (RE-046), and in scanned lists (RE-047); unresolved `G_SETTIMG(0)` is 0. Intensity textures generate the CLUT `choose_psm` picks for them. **151 textures carry mip levels**, generated by box-filtering to the nearest palette entry so CI4 stays 4bpp, with the chain stopping where swizzling would be lost (RE-053). Texture VRAM 717 KiB, deliberately over the 700 KiB all-at-once figure. 30 failures remain: 26 are the loading-break transition's runtime framebuffer photocopy (RE-055, confirmed against the decompilation — not a converter gap), 4 CI textures whose material carries no palette (RE-056, lead only). **Dream Land's canopy still looks wrong** and mipmaps did not fix it (RE-053) |
