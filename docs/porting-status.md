...
| **M2 — Resource pipeline** | ✅ COMPLETE | Archive + VPK0 verified; 2450 meshes (42,417 triangles, 0 conversion failures) and 617 textures packed into a 2.8 MB runtime pack that round-trips |
...
| Texture → PSP conversion | 🟢 90% | **647 bound, 617 packed**, 75.3% VRAM saved, **577.7 KiB — fits texture VRAM all at once**. Cross-file pointers resolve through the archive's extern relocations in the display list (RE-037), in a material's sprite table (RE-046), and in lists found by scanning, which had been decoded at base 0 so every relocation lookup missed and read as an unresolved null (RE-047). Unresolved `G_SETTIMG(0)`: 75 → **0**. Intensity textures now generate the CLUT `choose_psm` picks for them instead of falling through to 8888. 30 failures remain: 26 screen wipes textured with a runtime screenshot, and 4 CI textures whose material carries no palette. Wrapping textures are sized from their tile mask, not their drawn rectangle (RE-044) |
...
| Stage animation | 🟡 45% | The 32-bit `AObjEvent32` joint stream stages use is decoded and played into the same `AObj` tracks figatree drives — a different encoding, the same machine, but with real `f32` values rather than scaled `s16`. **All 206 scripts across all 41 stages replay, and all 206 are still looping after 600 frames**, which is the check that catches a one-word desync; "no unmodelled opcode" and "no denormal values" both pass even when the stream *is* slipped (RE-050). Not packed and not ticked on device, so Whispy still does not sway. Material animation (12 layers) is read but not played; its frame 0 matches the baked colours, so nothing renders wrong (RE-048) |
