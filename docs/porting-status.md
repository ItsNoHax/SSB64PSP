...
| Billboard nodes | 🟢 97% | 109 nodes flagged; RE-131–133 shipped the real camera, screen-aligned camera basis and distinct Kind48 basis (`FLAG_BILLBOARD_PITCH_LOCKED`, 47 nodes). Kind50 is unused in the archive. RE-134 measured uniform ancestor scales for every billboard. RE-136/137 spot-checked all 8 stages containing billboards under PPSSPP; per-node visual correctness remains open. RE-140's repeatable `billboard_inventory` example identifies every node by source file/graph/local index and reports 109 finite, nonzero rest-pose bases. RE-141 corrected Kind46 spin to Z with a separate v19 flag; kinds 44/48/50 do not spin. Nonzero-angle regression passed; current Dream Land capture unchanged (0 differing pixels). Animated spin and per-node visual checks remain open. Structural evidence does not establish visual or physical PSP correctness. |
