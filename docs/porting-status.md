...
| Billboard nodes | 🔴 0% | `DObjDesc.id & 0xF000` selects a matrix kind, and kinds 45–48 build the MVP from the projection matrix with the cross terms zeroed — screen-aligned billboards. `scene::transform_kind()` parses this and is unit-tested, but is called nowhere: **81 nodes are camera-relative and all are drawn with a static matrix** (34 `Kind46`, 47 `Kind48`), which is why six sprites sit flat in Dream Land's canopy. 28 more want `RecalcRotRpyRSca` (RE-048) |
