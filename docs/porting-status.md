...
| Base moveset (Mario) | 50% | Mario's ground+aerial moveset is complete except specials: jab (now the full `Attack11`→`Attack12`→`Attack13` chain — see below), dash attack, all forward-tilt/-smash angles, up/down tilt, up/down smash, all five aerials — ported field-for-field from `dMarioMainMotion_*` (`relocData/202_MarioMainMotion.c`). Discovered while researching smashes: **SSB64 smashes have no charge mechanic at all** (that is a Melee addition) — a smash is just a hard flick (`|stick| >= 56` inside a short tap window) versus a tilt's plain magnitude push, so smashes are the same shape as tilts. Hit resolution is a per-`(FighterKind, Status)` `MoveData` table (`crate::attack::move_data`) carrying every hitbox and its own active-frame window — `apply_hit_from` works for any ported status/fighter pair, including a hitbox's `ox` mirrored by facing, multiple simultaneous hitboxes, weakening hitbox phases, and a genuinely *pulsing* hitbox (down aerial: 16 real disjoint windows). Aerial landing takes a dedicated `LandingAirX` status or a real-percentage-scaled `LandingAirNull`. **The jab combo now reaches its real finisher, `Attack13`**, via a new architectural piece: `crate::status::AnyStatus` — `Common(Status)` for the shared 0..=219 table, or `Mario(MarioStatus)` (currently just `Attack13 = 220`) for a fighter's own extended statuses above it, since the original numbers every fighter's own table independently from 220 and a flat shared enum would either collide fighters' ordinals or need fidelity-breaking renumbering. `AnyStatus` is now what `StatusState`/`Fighter` actually store; a `PartialEq<Status> for AnyStatus` impl means the overwhelming majority of this crate's existing comparisons (`f.status.status == Status::Wait`) needed no changes at all — only the handful of places that *pattern-match* on the field (`update`'s big dispatch, `move_data`, a couple of others) needed a one-line `let AnyStatus::Common(s) = ... else { ... }` unwrap or an `AnyStatus::Common(...)`-wrapped arm. `attack13_status(kind)` gates which fighters even have one, mirroring `ftCommonAttack13CheckFighterKind` instead of assuming every fighter does | — | `Attack100` (rapid-jab loop, a different mechanic for a different fighter set — `ftCommonAttack100CheckFighterKind` excludes Mario) not ported; no other fighter has a `MarioStatus`-equivalent enum or any moveset data yet; specials; grabs/throws; `DTilt`'s repeated-tap extension; the narrow `anim_frame <= 5` pivot-smash-out-of-dash-startup window; `LandingAirF`/`Hi`/`B`/`Lw` collapsing to one tick; the real aerial auto-cancel branch | P2 |
