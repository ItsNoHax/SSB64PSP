...
315 host tests passing across `ssb-rom` (172), `ssb-engine` (36) and
`ssb-game` (107).
...
5. **Animation is validated but not yet driven by the game.** The whole path
   runs on device and is checked four ways (RE-038), but only the viewer plays
   it: `Fighter::tick` still does not start an animation when it changes
   status, so a fighter on a stage is a sliding rest pose. That is the next
   concrete task — wiring the status machine's slot to `Skeleton::start`, with
   the playback speed the status supplies (0.5 for a heavy landing, RE-035).
   Every status it can be in now has an animation to start (RE-041).
