...
| Texture → PSP conversion | 🟢 96% | **665 bound, 638 packed**, 69% VRAM saved. Cross-file pointers resolve through the archive's extern relocations in the display list (RE-037), in a material's sprite table (RE-046), and in scanned lists (RE-047); unresolved `G_SETTIMG(0)` is 0. Intensity textures generate the CLUT `choose_psm` picks for them. **151 textures carry mip levels**, generated by box-filtering to the nearest palette entry so CI4 stays 4bpp, with the chain stopping where swizzling would be lost (RE-053). Texture VRAM 763 KiB, deliberately over the 700 KiB all-at-once figure. 27 failures remain, both root-caused outside the converter: 26 are the loading-break transition's runtime framebuffer photocopy (RE-055, confirmed against the decompilation — `PLAN.md` R0.13's scope), 1 is an `MObj` material-table pairing gap in the one remaining file (RE-057/RE-060 — `PLAN.md` R0.7's scope; two other files, Link's entrance effects and the opening movie's room scene, were fixed this session by hand-pairing two previously-unscanned record shapes). **Dream Land's canopy still looks wrong** and mipmaps did not fix it (RE-053) |
...
| PSP mesh drawing | 🟢 85% | Indexed GE draws, CLUT textures, wrap, baked shading, per-node baked matrices. Yoshi's 28-joint hierarchy at 60 FPS, 589us CPU. **The hardcoded `sceGuTexWrap(Repeat, Repeat)` is verified correct, not a placeholder** — measured archive-wide that every clamp-flagged tile-0 axis is also a masked (periodic) one, so the existing mask-narrowed texture size already reproduces real TMEM addressing under `Repeat` (RE-044, RE-066). `G_TX_MIRROR` (27.6% of tile-0 lists) has no GE equivalent and remains an accepted, quantified deviation |
