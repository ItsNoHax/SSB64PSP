...
| Texture → PSP conversion | 🟢 96% | **696 bound, 667 packed**, 51.3% VRAM saved (latest `romtool textures`, RE-157). Cross-file pointers resolve through the archive's extern relocations in the display list (RE-037), in a material's sprite table (RE-046), and in scanned lists (RE-047). The rebuilt pack has **318 mip-carrying texture entries**; mirrored-copy pre-baking remains how the PSP's `Repeat`-only wrap reproduces `G_TX_MIRROR` exactly (RE-067). Texture VRAM is **1287.8 KiB, 1.8x the ~700 KiB all-at-once budget**, so streaming remains required. 29 failures are understood: 26 are the loading-break transition's runtime framebuffer photocopy (RE-055/R0.13); the remaining three `MissingPalette` cases are two in file 114 and one in file 86, still owned by R0.7 material-table recovery. **Dream Land's canopy discrepancy is partially resolved** — mirror-wrap is fixed and dither is softer (RE-070), but the magnification diagnosis remains open. |
