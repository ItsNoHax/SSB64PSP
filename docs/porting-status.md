...
| Texture → PSP conversion | 🟢 96% | **686 bound, 657 packed**, 52.3% VRAM saved (latest `romtool textures`, RE-153). Cross-file pointers resolve through the archive's extern relocations in the display list (RE-037), in a material's sprite table (RE-046), and in scanned lists (RE-047). **309 of 657 textures carry mip levels**; **187 of 657 (28%) are pre-baked with a mirrored copy** (`crates/ssb-rom/src/texture.rs::mirror_extend`) so the PSP's `Repeat`-only wrap reproduces `G_TX_MIRROR` exactly (RE-067). Texture VRAM is **1238.2 KiB, 1.8x the ~700 KiB all-at-once budget**, so streaming remains required. 29 failures are understood: 26 are the loading-break transition's runtime framebuffer photocopy (RE-055/R0.13); the remaining three `MissingPalette` cases are two in file 114 and one in file 86, still owned by R0.7 material-table recovery. **Dream Land's canopy discrepancy is partially resolved** — mirror-wrap is fixed and dither is softer (RE-070), but the magnification diagnosis remains open. |
