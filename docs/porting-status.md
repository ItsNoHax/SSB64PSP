...
| Recovery (`FallSpecial`) | 20% | `ftCommonFallSpecialSetStatus`/`ftCommonLandingFallSpecialSetStatus` ported (`crate::status::set_fall_special`/`set_landing_fall_special`, `FallSpecialState`) — the shared "helpless fall after a recovery move's launch phase" status every fighter's up-special (and some others) enters, reusing every physics primitive already in `physics.rs` (`apply_gravity_clamp_tvel`, `check_clamp_air_vel_x_dec`, `clamp_air_vel_x_stick_range`, `apply_air_friction` — all already existed from the pre-batch-mode physics work, ahead of when this batch needed them). Spends every remaining jump on entry (matches the real "no double-jump after up-B" rule) and only allows a jump-cancel interrupt, never an aerial attack, matching `ftCommonFallSpecialProcInterrupt` exactly. Landing correctly distinguishes "skip landing lag" from a real `LandingFallSpecial` recovery pose by `is_goto_landing`/fall speed | — | **Mario's own `SpecialHi` (`SuperJumpPunch`) is not wired on top of this yet.** Investigation found the actual rise/launch velocity for Mario's up-B is driven by the *animation file's own root-motion data* (`ftPhysicsApplyAirVelTransNAll`/`...GroundVelTransN`, reading per-frame translation from the clip) — this codebase's animation pipeline does not extract that data yet (`docs/porting-status.md`'s Animation row: "No `translate_scales`" is exactly this gap). Inventing a velocity arc instead of extracting the real one would violate decomp-fidelity, so `SpecialHi` waits on that prerequisite. `is_allow_pass`'s drop-through-platform nuance during the fall, and cliff-catch-while-falling (no status auto-catches a ledge yet — a pre-existing gap, not new here), are also not ported | P2 |
