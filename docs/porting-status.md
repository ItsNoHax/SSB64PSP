...
| Texture → PSP conversion | 🟡 96% / VERIFYING | ROM-backed conversion, mirror pre-baking, clamp/origin lowering and RE-201's Dream Land hardware capture are complete for their covered paths. Texgen's T1–T10 tile-shift/addressing and raw-coordinate audit remains open; current pack format is version 27. |
| DL discovery | ✅ COMPLETE | 1,864 lists across 135 files; converter used as validator (RE-017) |
| Mesh conversion | 🟡 86% / VERIFYING | 0 archive-wide conversion failures; combiner classification, material threading and current lighting paths are implemented. RE-217 reopens validation for single-source `PRIM * SHADE`, `G_VTX` load-time lighting provenance, independent depth compare/write state and global primitive reordering; see `PLAN.md` R2.2/C1–C4. |
| Model conversion | 🟡 80% / VERIFYING | Meshes extracted, DObj hierarchy applied, all 127 discovered material graphs paired (RE-163), and primitive colours applied on the covered combiner paths. RE-217's C1/C2 review must revalidate runtime-lit/literal ownership and load-time vertex semantics across the named mixed-material fighters. |
...
| PSP mesh drawing | 🟡 90% / VERIFYING | Indexed GE draws, CLUT textures, addressing, animation, alpha gates, runtime lighting and texgen are implemented and exercised in PPSSPP/physical-PSP scenes. RE-226/RE-227/RE-228 close raw-normal semantics, LookAt quantization, and shared regular/linear texgen reference math (the last of which also found and fixed a real overcorrected matrix constant); independent depth writes, submission order, full DrawState cache isolation and the rest of the `R2.1` texgen queue (T1's cross-node gap, T5–T10) remain open. |
...
1. **Physical PSP hardware validation is still in progress.** PPSSPP is not
   proof of hardware behaviour (`AGENTS.md` §16). RE-201–215 now record
   representative PSP Slim/6.61 captures for the current renderer, including
   texgen, but exhaustive coverage, the original-N64 Metal comparison and the
   R2.2 corrective regressions remain open. Treat every "on device" claim
