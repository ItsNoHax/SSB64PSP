...
| Texture → PSP conversion | 🟢 96% | **665 bound, 638 packed**, 56.5% VRAM saved. Cross-file pointers resolve through the archive's extern relocations in the display list (RE-037), in a material's sprite table (RE-046), and in scanned lists (RE-047); unresolved `G_SETTIMG(0)` is 0. Intensity textures generate the CLUT `choose_psm` picks for them. **151+ textures carry mip levels**, generated by box-filtering to the nearest palette entry so CI4 stays 4bpp, with the chain stopping where swizzling would be lost (RE-053). **187 of 638 (29%) are pre-baked with a mirrored copy** (`crates/ssb-rom/src/texture.rs::mirror_extend`) so the PSP's `Repeat`-only wrap reproduces `G_TX_MIRROR` exactly rather than approximating it (RE-067) — traced directly to Dream Land's canopy display list. Texture VRAM **1059 KiB, 1.5x the ~700 KiB all-at-once budget** (was 763 KiB before RE-067; shipped deliberately, over the earlier 763 KiB figure that was itself already over-budget, per explicit user decision — texture streaming is no longer optional). 27 failures remain, both root-caused outside the converter: 26 are the loading-break transition's runtime framebuffer photocopy (RE-055, confirmed against the decompilation — `PLAN.md` R0.13's scope), 1 is an `MObj` material-table pairing gap in the one remaining file (RE-057/RE-060 — `PLAN.md` R0.7's scope). **Dream Land's canopy discrepancy is partially resolved** — the mirror-wrap contributing cause is fixed and verified (RE-067), but RE-053's separate magnification/dithering diagnosis remains open |
