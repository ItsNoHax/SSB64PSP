...
| PSP mesh drawing | 🟢 88% | Indexed GE draws, CLUT textures, measured repeat/mask/mirror/clamp addressing, baked shading, per-node matrices, and per-primitive depth. Pack version 22 preserves normals and source LIGHT_1/LIGHT_2 values; the GE now configures the original stage-angle directional light only around fighter draws and enables it only for `LIT` primitives (RE-165). PPSSPP exposes an unresolved too-dark fighter, so this remains an active correctness investigation. Alpha-tested cutouts render correctly (RE-069); RE-129/130 enable real source-alpha blending for the two classified single-cycle alpha formulas while declining the measured rare/two-cycle long tail. RE-144 restores the original billboard X/Y/X scale rule and adds isolated per-node inspection. |
...
434 host tests passing across `ssb-rom` (280), `ssb-engine` (36) and
