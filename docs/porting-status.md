...
| Texture → PSP conversion | 🟢 85% | **695 bound, 612 packed**, 72.7% VRAM saved. **Cross-file texture pointers now resolve** through the archive's extern relocations, which is what stages reach their texels by: Dream Land went from 1 texture to 18 and renders textured on device (RE-037, `docs/images/m4-stage-textured.png`). 83 references still fail — 54 null pointers nothing names (which is what Dream Land's ground still draws white for, RE-045), 29 paletted without a recorded TLUT, 16 missing a palette at decode, 13 segmented. Round-trip tested through swizzle+CLUT; verified on device (RE-022). **At 607 KiB the full set fits texture VRAM all at once**: it had been measured at 1.1 MiB because wrapping textures were converted at the size they were *drawn* rather than the size they are (RE-044), so the streaming requirement was an artefact |
| DL discovery | ✅ COMPLETE | 1,864 lists across 135 files; converter used as validator (RE-017) |
| Mesh conversion | 🟢 85% | 47,696 tris, vertex dedup and material merging; 0 failures archive-wide. **Primitive colours are folded into the vertex shade** the way the N64 combiner does (`PRIM * SHADE`), gated on `G_SETCOMBINE` actually naming `PRIMITIVE` — without that gate Mario's white gloves came out green (RE-039). **Both combiner cycles are evaluated** into a scale on the vertex shade, declining anything that is not one rather than approximating it (RE-043). Dream Land now packs 18 of the 19 textures it binds and its ground draws textured (RE-044). Cross-joint vertex sharing resolved for the rest pose (RE-026); no animated skinning yet |
