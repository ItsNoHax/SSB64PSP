...
| PSP mesh drawing | 🟢 87% | Indexed GE draws, CLUT textures, wrap, baked shading, per-node baked matrices. Yoshi's 28-joint hierarchy at 60 FPS, 589us CPU. **The hardcoded `sceGuTexWrap(Repeat, Repeat)` is verified correct, not a placeholder** — measured archive-wide that every clamp-flagged tile-0 axis is also a masked (periodic) one, so the existing mask-narrowed texture size already reproduces real TMEM addressing under `Repeat` (RE-044, RE-066). `G_TX_MIRROR` (29% of packed textures) is now reproduced exactly by pre-baking a mirrored copy at pack time rather than approximated (RE-067) |
