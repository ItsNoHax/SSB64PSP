...
| Scene graph (DObj) | 🟢 85% | All 363 `DObjDesc` arrays recovered and validated against the decomp (RE-023); world transforms baked into the pack. Three union members of `DObj`'s display-list field resolved, and node lists converted in draw order through one shared vertex cache — zero conversion failures archive-wide (RE-025, RE-026). `MObj` material chains recovered for **104 graphs (23 still unpaired)** via `FTCommonPart`, `MPGroundDesc`, `EFDesc` and source-confirmed call-sequence records, including all ten character-select/result emblems and four source-paired common effects (RE-153/154); 424 paired nodes have zero chain/demand mismatches. `GObj` layer and animation still absent |
