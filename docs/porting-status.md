...
| PSP mesh drawing | 🟢 88% | Indexed GE draws, CLUT textures, measured repeat/mask/mirror/clamp addressing, baked shading, per-node matrices, and per-primitive depth. Pack version 21 preserves signed N64 normals and declares `GU_NORMAL_8BIT`, the prerequisite for replacing baked lighting with the original runtime directional path (RE-164); GE lighting is not enabled until lit/literal primitive separation is verified. Alpha-tested cutouts render correctly (RE-069); RE-129/130 enable real source-alpha blending for the two classified single-cycle alpha formulas while declining the measured rare/two-cycle long tail. RE-144 restores the original billboard X/Y/X scale rule and adds isolated per-node inspection. |
...
433 host tests passing across `ssb-rom` (279), `ssb-engine` (36) and
