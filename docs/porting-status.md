...
| Texture → PSP conversion | COMPLETE for measured renderer scope | Mirror/clamp/origin/signed-coordinate lowering, palette banks, texgen T1–T10, PSP sample-centre lowering, and selective build-time 3-point compensation (material-aware on alpha; CI4/CI8 use filter-aware index coordinate descent; animated CI shares one index field optimized over reachable `PaletteID` states; direct colour has GE-exact integer refinement, RE-309) are measured and complete. Pack format v30 | RE-201, RE-219–239, RE-262–269, RE-301, RE-304–309 | Exact 3-point equality remains impossible with fixed-function bilinear; measured admission gates retain source indices where joint optimization is insufficient | R0.5, R2.0 |
