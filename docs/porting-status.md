...
| Fighter state | 63% | The full `FTCommonStatus` ordinal table (0..=219, `ft/ftcommon/ftcommonstatus.h`) is now in `Status`, matching the decomp's own numbering exactly. Movement status machine (Wait/walks/Dash/Run/Turn/Jump/Fall/Squat/Landing/Pass) unchanged. The Damage/hitstun family (`DamageHi/N/Lw1-3`, `DamageAir1-3`, `DamageFlyN`/`FlyTop`, `DamageFall`) now has real entry/exit: a landed hit picks its status from `ftCommonDamageGetDamageLevel`'s hitstun tiers and the defender's grounded/airborne situation, and hitstun running out returns to `Wait` or `DamageFall` (`ftCommonDamageCommonProcInterrupt`/`...AirCommonProcInterrupt`). Plus, under `F1`'s scoped Training-only exception (`AGENTS.md`): Mario's neutral jab (`Attack11`), input to hitstun, pixel-confirmed landing on the real dummy target. Mario's `SpecialHi`/`SpecialAirHi` ordinals and real clip slots are now represented, but their callbacks are not | RE-033, RE-035, RE-294, RE-295, RE-299 | Every status past the wired movement/Damage/base-moveset subset exists as an ordinal only, with no callback behaviour (specials, grabs, smashes, aerials besides what's listed below — `P2` scope). Within Damage: no hit-location Hi/Lw index (every hit reads as "N"/middle), no `DamageFlyRoll` (needs a shared RNG source), and the ground-hit-still-launches-airborne angle branch is dropped (`Status::is_grounded`'s docs) | M3, G0 |
| Base moveset (Mario) | 50% | Mario's ground+aerial moveset is complete except specials: jab (the full `Attack11`→`Attack12`→`Attack13` chain), dash attack, all forward-tilt/-smash angles, up/down tilt, up/down smash, all five aerials — ported field-for-field from `dMarioMainMotion_*` (`relocData/202_MarioMainMotion.c`). Discovered while researching smashes: **SSB64 smashes have no charge mechanic at all** — a smash is just a hard flick inside a short tap window versus a tilt's plain magnitude push. Hit resolution is a per-`(FighterKind, Status)` `MoveData` table (`crate::attack::move_data`) carrying every hitbox and its own active-frame window. The jab combo reaches its real finisher, `Attack13`, via `crate::status::AnyStatus` (`Common(Status)` for the shared table, `Mario(MarioStatus)` for a fighter's own extended statuses above ordinal 219 — every fighter numbers its own extended table independently in the original, so a flat shared enum would collide fighters against each other) | — | `Attack100` (rapid-jab loop, excludes Mario); no other fighter has a `MarioStatus`-equivalent enum or any moveset data yet; specials; grabs/throws; `DTilt`'s repeated-tap extension; `LandingAirF`/`Hi`/`B`/`Lw` collapsing to one tick; the real aerial auto-cancel branch | P2 |
| Recovery (`FallSpecial`) | 20% | `ftCommonFallSpecialSetStatus`/`ftCommonLandingFallSpecialSetStatus` ported (`crate::status::set_fall_special`/`set_landing_fall_special`, `FallSpecialState`) — the shared "helpless fall after a recovery move's launch phase" status every fighter's up-special (and some others) enters, reusing every physics primitive already in `physics.rs` (`apply_gravity_clamp_tvel`, `check_clamp_air_vel_x_dec`, `clamp_air_vel_x_stick_range`, `apply_air_friction` — all already existed from the pre-batch-mode physics work, ahead of when this batch needed them). Spends every remaining jump on entry (matches the real "no double-jump after up-B" rule) and only allows a jump-cancel interrupt, never an aerial attack, matching `ftCommonFallSpecialProcInterrupt` exactly. Landing correctly distinguishes "skip landing lag" from a real `LandingFallSpecial` recovery pose by `is_goto_landing`/fall speed | RE-299 | **Mario's own `SpecialHi` (`SuperJumpPunch`) is not wired on top of this yet.** Its 40-frame TransN root-motion clip is now packed and ROM-verified; `translate_scales` is unrelated (Mario has none). The missing layer is portable-gameplay access to the runtime skeleton's hidden TransN pose, followed by the move's sourced motion events/callbacks. `is_allow_pass`'s drop-through-platform nuance during the fall, and cliff-catch-while-falling (no status auto-catches a ledge yet — a pre-existing gap, not new here), are also not ported | P2 |
...
| Animation | 90% | Figatree scripts decode to per-joint transforms and are packed; 189 movement animations plus Mario's 40-frame Super Jump Punch, including its hidden TransN entry, replay exactly against the ROM; skeleton ticks at 60 FPS on device; all 534 sparse fighter/slot entries replay correctly | RE-036, RE-038, RE-171, RE-299 | `translate_scales` is not required by Mario's up-B; gameplay cannot yet consume a hidden runtime-joint pose, and viewer camera frames on rest bounds only | M3 |
