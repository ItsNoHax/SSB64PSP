...
| Effects / particles | 🟡 55% | RE-172 inventories the original manager's 53 `EFDesc` records: three are controller-only, four pairs share assets, and all 46 unique display-bearing DObj/direct-list effects survive the pack (`romtool effects`: 46/46, 668 triangles). RE-173 captures every rest identity. RE-174 packs and host-replays all 35 descriptor-selected transform `AObjEvent32` tables (65 nodes), then PPSSPP-captures all 35 frame-4 poses with unique HUDs: 34 visible and Link Spin Attack correctly hidden pending its material alpha stream. | Effect material `AObjEvent32` playback and facing-dependent alternate streams remain. The independent `LBParticle` script/texture-bank runtime used by dust, flame, sparkle, hit and stage effects is not implemented. |
