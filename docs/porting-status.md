...
| Animation | 92% | Figatree scripts decode to per-joint transforms and are packed; 189 movement animations plus Mario's 40-frame Super Jump Punch, including its hidden TransN entry, replay exactly against the ROM; skeleton ticks at 60 FPS on device and now supplies TransN delta/pitch to portable gameplay; all 534 sparse fighter/slot entries replay correctly | RE-036, RE-038, RE-171, RE-299 | `translate_scales` is not required by Mario's up-B; viewer camera frames on rest bounds only | M3 |
