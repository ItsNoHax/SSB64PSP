...
| Effects / particles | 🟡 45% | RE-172 inventories the original manager's 53 `EFDesc` records: three are controller-only, four pairs share assets, and all 46 unique display-bearing DObj/direct-list effects now survive the pack (`romtool effects`: 46/46, 668 triangles). File-84's shifted Catch Swirl/Reflect Break/Dead Explode/Ness PK Flash material associations are corrected from current decomp and ROM evidence. RE-173's PPSSPP-software audit captures every source identity: 43/43 authored-visible objects contain measurable centred pixels; Ness PK Flash, Samus Entry Point and Link Spin Attack reproduce their source-authored invisible rest poses. | Effect `AObjEvent32` playback remains. The independent `LBParticle` script/texture-bank runtime used by dust, flame, sparkle, hit and stage effects is not implemented. |
