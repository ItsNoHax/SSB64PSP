...
| Stage animation | 🔴 0% | `MPGroundDesc`'s `anim_joints` (+4) and `p_matanim_joints` (+0xC) are now read — **40 of 100 layers animate joints, 12 animate materials** — but nothing plays either, so Whispy never sways. The materials are not wrong, only still: every stage's matanim frame 0 was decoded and matches the baked `MObjSub` colours exactly, so the RE-040 initial-value bug does not apply here (RE-048) |
