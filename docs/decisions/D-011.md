...
**Implemented:** `crates/ssb-rom/src/pack.rs` (manifest); closure layout and patching in `crates/ssb-rom/src/reloc_link.rs` (RE-340), checked against `lbRelocGetAllocSize` and live N64 RDRAM placement (RE-341). Runtime use is still TODO.
