...
* [ ] environment color verified — same post-RE-163 remeasurement is required
  ...
  evidence.
* [ ] lighting verified — RE-103/RE-105 fixed the input this still depends on (per-vertex, not per-primitive-majority, lit/literal decisions, driven by a real `G_MW_LIGHTCOL` ROM signal rather than a guess). RE-164 retained stage X/Y angles and signed normals; RE-165 additionally retains the real directional/ambient LIGHT_1/LIGHT_2 values, scopes GE lighting to fighter draws, and disables it for literal primitives. It remains open because the first PPSSPP capture is too dark: source must establish the inherited initial ambient register before a brightness approximation is considered. RE-152 confirms RE-128's Fox black patch was unrelated: it was caused by nonzero clamp-window coordinates
