...
in `Gpu::draw_triangles` along the way), and a 10-minute sustained run. The
formal hardware matrix still lacks PSP-1000 and coverage beyond the 22
committed deterministic goldens and this session's 10-minute sample; those
remain deliberately deferred until the game structure grows beyond the
current asset viewer, and keep R3 blocked until resumed.
...
* [ ] no hardware-only rendering failures remain — five golden scenes (Dream Land/Mario, `MVOpeningRoom`, `StageSectorFile2`, `CatchSwirl`, Saffron City) plus fighter, texgen, and plain-interactive checks are clean. RE-265 adds exact PPSSPP software goldens for all 12 playable fighters in their high-detail Wait pose with runtime fighter lighting; RE-270 physically re-confirms Fox/Ness/Link/Donkey Kong/Dream Land against RE-262/264/269's fixes; RE-271 physically confirms the remaining 6 playable fighters, re-confirms Kirby, physically runs the depth-mask diagnostic for the first time (finding and fixing a real `Gpu::draw_triangles` GE race along the way), and sustains 10 continuous minutes with zero exceptions — all 22 committed deterministic goldens are now physically confirmed, but coverage across all 41 stages/effects (no dedicated capture scenes exist for most of them) and PSP-1000 remain open, deliberately deferred until the game structure grows beyond the current asset viewer
