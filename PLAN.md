...
| Depth/culling correctness (depth direction/range/function/writes, polygon culling, winding, clipping) | R0.6 / R0.14 / R2.2-C3 | `VERIFYING` — RE-244 (`R2.2`/C3, part 1) added independent `depth_test`/`depth_write`/`depth_mode` fields and measured a real 90% archive-wide divergence from the single `z_buffer` flag PSP rendering still uses; RE-245 (part 2) found and wired the same external per-object depth seed for stage render-layer 1, closing 577 of the 4468-primitive gap (to 3891); RE-246 (part 3) found the dominant remainder was a *camera*-level default (not an object wrapper) and wired it for the 11 loading-break transition scenes, closing 1951 more (to 1940); RE-247 (part 4) found `ssb_rom::transition::ASSETS`'s `"camera"` entry pointed at the wrong file (a real pack-content bug, not just an attribution gap — the shipped "Camera Shutter" results-wipe was packing the wrong geometry), fixed it, and closed 132 more (now 1808); RE-248 (part 5) confirmed file 39 (`IFCommonObject`) is orphaned geometry no code path ever draws, explaining 185 of the 1808 with no seed needed; RE-249 (part 6) ruled out any further external wrapper archive-wide (no dominant cluster; a 37-file `Stage*File2+`/`GRBonus*File2` cluster and several fighter-submodel/item files traced directly turn out to be real in-content `G_ZBUFFER`-without-`Z_CMP`/`Z_UPD` divergence, the exact case C3 exists to capture, not a missing seed); RE-250 (part 7) gave `PlannedList` its own `list_id` field and a per-item `SequenceItem::depth_seed` override, so stage render-layer 1's 21 list-1 (translucent) `DObjDLLink` entries now seed depth-test-without-write instead of the uniform opaque write-on seed (`depth_write` true count for layer 1 falls 776→668) — PSP-side `sceGuDepthMask` wiring and device evidence remain open |
...
RE-243); C3 in progress (RE-244, RE-245, RE-246, RE-247); C4–C7 remain, depend on R2.1/T1–T10
...
Status: `IN_PROGRESS` (RE-244 part 1, RE-245 part 2, RE-246 part 3, RE-247 part 4, RE-248 part 5, RE-249 part 6, RE-250 part 7). Census `Z_CMP`, `Z_UPD`,
...
Remaining for C3: decide the actual PSP-side `sceGuDepthMask` wiring now
that the gap is understood to be real content plus this now-fully-corrected
seed, not a missing seed; and add the depth-test/no-write
translucent-front/opaque-behind scene plus ON→OFF→ON regression with device
evidence. Until this closes, depth is not complete.
