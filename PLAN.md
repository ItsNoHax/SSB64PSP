...
Status: `IN_PROGRESS` (RE-244 part 1, RE-245 part 2, RE-246 part 3). Census `Z_CMP`, `Z_UPD`,
...
Remaining for C3: file 39 (`IFCommonObject`) is still unattributed — find
its real draw path (`refs/ssb-decomp-re/src/if/ifcommon.c`/
`ifscreenflash.c` not yet read closely); find (or rule out) any other
object category's or camera's own external wrapper (layers 0/2/3, items,
effects, and whatever part of the main battle camera's own first-drawn-
object default is ever statically distinguishable); give `PlannedList` its
own `list_id` field so layer 1's list-1 entries can be seeded correctly;
decide the actual PSP-side `sceGuDepthMask` wiring once the gap is
understood; and add the depth-test/no-write translucent-front/opaque-
behind scene plus ON→OFF→ON regression with device evidence. Until this
closes, depth is not complete.
