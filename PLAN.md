...
* [ ] environment color verified — same post-RE-163 remeasurement is required
  ...
  evidence.
* [ ] lighting verified — RE-103/RE-105 fixed the input this still depends on (per-vertex, not per-primitive-majority, lit/literal decisions, driven by a real `G_MW_LIGHTCOL` ROM signal rather than a guess). RE-164 establishes that the former RE-065 baked-key-light deviation is removable: the pack now retains the original stage X/Y angles and signed normals, and the PSP vertex layout consumes them. It remains open because enabling primitive-level GE lighting before a source-verified lit/literal draw split would reinterpret authored literal colours as normals. RE-152 confirms RE-128's Fox black patch was unrelated: it was caused by nonzero clamp-window coordinates
