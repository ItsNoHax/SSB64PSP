...
Status: `IN_PROGRESS` (RE-244 part 1, RE-245 part 2, RE-246 part 3, RE-247 part 4, RE-248 part 5, RE-249 part 6). Census `Z_CMP`, `Z_UPD`,
...
Remaining for C3: give `PlannedList` its own `list_id` field so layer 1's
list-1 entries can be seeded correctly; decide the actual PSP-side
`sceGuDepthMask` wiring now that the gap is understood to be real content
rather than a missing seed; and add the depth-test/no-write
