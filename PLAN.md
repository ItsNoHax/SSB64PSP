...
* [x] texture coordinate behavior verified — RE-128: `TEXVIEW`, the debug viewer's direct texture-display mode (bypasses lighting/geometry entirely), confirms Fox's real face texture (index 550) and Kirby's real face texture (index 734) both display correctly on-device — matching `romtool texdump`'s independent reference decode exactly, no melting, no clamp-boundary seam. RE-101/RE-102 are now independently re-verified against real fighter screenshots, closing the gap both entries left open
...
**RE-128 also found a real, separate, unexplained defect while verifying
the item above**: Fox's face shows a large, solid, confirmed-`(0,0,0)`
black patch in the full lit object-view render (not present in `TEXVIEW`,
so it is not a texture problem — confirmed by decoding the packed bytes
directly and by ruling out a swizzle bug). Two specific hypotheses
(texture corruption; a double-application of `prim_color`'s scale
crushing `shade_normal`'s ambient-floored grey to zero) were tested via
direct ROM display-list decoding and eliminated, not guessed away — see
RE-128's own entry. Not resolved; filed as a lead on `R0.6`'s already-open
"primitive color verified"/"lighting verified" items below, not as a new
acceptance item here, since this task's own texture-coordinate question
is answered independently of it.

### Evidence

RE-044, RE-053, RE-066, RE-067, RE-070, RE-075, RE-081, RE-101, RE-102, RE-127, RE-128 in `docs/reverse-engineering.md`.
...
* [ ] primitive color verified — RE-079's census plus RE-080's `combiner_flat_color` now cover every shape this model resolves at all: shade-scale, texture-blend and flat-constant are structurally disjoint and together account for 97.5%+ of archive-wide combiner-bearing primitives. What remains open is not a classification gap: `(PRIM-ENV)*TEXEL0+ENV`'s 3,085/4,580 misses are a genuine absence of `prim_color` on this converter's own per-graph state (likely `R0.7`'s material-table pairing gaps), which no combiner-shape work can fix. RE-106 additionally found and fixed a *consumption* gap in the cases that already classified correctly: `prim_color` was resolved but never multiplied into the actual vertex, baked in now. **RE-128 found a real, on-device instance of a fighter (Fox) rendering a large solid-black facial region while verifying `R0.5`'s texture-coordinate item.** A double-application of `prim_color`'s scale (once in `mesh.rs::push_vertex`, again in `pack.rs::add_mesh`'s own `prim_scale` step) looked like exactly the right shape of bug, but decoding the real display list at the specific primitives involved found no `G_SETPRIMCOLOR` near zero anywhere nearby — their exported `prim_color` reads as the same bit pattern as "unset" because `add_object` cannot distinguish `None` from `Some(black)` in that field, and tracing to the source list confirms it really is `None` there. This hypothesis is eliminated for the checked primitives; the true mechanism remains open, and which exact primitive draws the visible pixels was not confirmed geometrically. A concrete, reproducible, currently-open lead, not a new task
...
* [ ] orientation verified — RE-126 measured this is a real, open gap, not an unexamined one: `Kind48` (camera-pitch-locked, distinct from `Kind46`'s fully screen-aligned transform) is 47 real nodes archive-wide including Dream Land's own file 104, the largest individual billboard category (43% of all 109 flagged nodes) — currently rendered with `Kind46`'s screen-aligned approximation instead of its own real pitch-locked one. Not yet visibly wrong on screen because neither RE-049's test camera nor the current, still-face-on gameplay camera ever varies along the axis the two transforms disagree on; blocked on R0.14's still-open "actual game camera" item, which this finding gives a concrete reason to need. **RE-131 built that camera** (`ssb_game::camera`, a real, tested, on-device-verified port of `gmCameraDefaultFuncCamera`), closing the literal blocker. **RE-132 wired the camera's own basis into the billboard path** (fixing a regression the camera itself introduced for `Kind46`, see that item's own entry below) but stopped short of `Kind48`/`Kind50`'s own distinct transforms: `gcPrepDObjMatrix`'s real matrices for those cases (`sGCMatrixMod1F`/`Mod2F`, `objdisplay.c` cases 47-50) depend on an unresolved branch (`var_s3`/`spC8`, selecting which world axis — X/Y vs. Y/Z — the lock applies to) that this pass did not have enough context to confidently resolve without guessing, which `AGENTS.md` §9 rules out. The concrete next step is now "resolve that one branch, then port the two LookAt-based matrices," not "build a camera" or "fix the billboard/camera interaction," both now done
...
RE-049, RE-062, RE-063, RE-083, RE-126, RE-131, RE-132 in `docs/reverse-engineering.md`.
