...
RE-243); C3 in progress (RE-244, RE-245, RE-246); C4–C7 remain, depend on R2.1/T1–T10
...
Remaining for C3: two files RE-099 flagged outside `dLBTransitionDescs` (39,
51) are still unattributed; find (or rule out) any other object category's
or camera's own external wrapper (layers 0/2/3, items, effects, and whatever
part of the main battle camera's own first-drawn-object default is ever
statically distinguishable); give `PlannedList` its own `list_id` field so
layer 1's list-1 entries can be seeded correctly; decide the actual PSP-side
`sceGuDepthMask` wiring once the gap is understood; and add the depth-test/
no-write translucent-front/opaque-behind scene plus ON→OFF→ON regression
with device evidence. Until this closes, depth is not complete.
