...
- [ ] **Graphs without material tables — 64 paired, 63 unpaired, and now characterized rather than a black box (RE-077).** Freshly re-measured (`romtool mobj`, no `--file` filter) after RE-059/RE-060's 7 fixes and RE-077's 1 more. Concrete, still-open cases: file 86 (`ITCommonObject`)'s one remaining graph (a fifth mechanism, see below), file 353's third graph (`SpinAttackDObjDesc @ 0x11C0`, a `WPAttributes` not yet typed in the decompilation), and Ness's one gap (a suggestive but not conclusive candidate) — see RE-057/RE-058/RE-060/RE-077. RE-077 also checked, rather than assumed, whether the other ~62 (now 61) untraced graphs are hiding in core fighter models: they mostly aren't. 9 of the 11 remaining real playable fighters (Mario, Fox, Donkey Kong, Samus, Luigi, Jigglypuff, Captain Falcon, Yoshi, Pikachu) have **zero** unpaired graphs of their own; the untraced ones are concentrated in character-select emblem models, stage files, and fighters' special-move files instead. Kirby's one core-model gap (22-node `JointTree_0x19F08`) was found via `--search` landing on exactly one demand-matching candidate (not the usual dozens) and confirmed against a fully-typed decompiled symbol before fixing.
