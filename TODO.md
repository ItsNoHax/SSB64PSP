...
- [ ] **MissingPalette (4)** — Palette pointer resolved but palette data not found in target file. Still 4 as of the last run. RE-056 has a lead, not a fix: at least one of the failing texture's several `G_SETTIMG` occurrences in its file *is* preceded by a matching `G_LOADTLUT` (file 52, offset 0x6530, palette source 0x1c0), but `romtool textures`' dedup key evaluates only the first occurrence it walks to, which may not be that one. Next step: confirm which occurrence `mesh::convert_sequence`'s actual walk order reaches first for each of the 4 cases, and whether `State::forget_texture()` (`crates/ssb-rom/src/mesh.rs`) is what clears the palette in between.
