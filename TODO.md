...
- [ ] **Stage material animation — scoped archive-wide before implementing (RE-086), not a colour-animation feature.** Hook `matanim_joints` into `draw_stage_animated` (12 layers, 172 individual scripts, `AObjEvent32`). Decoded but not played; frame 0 happens to match baked colours so nothing currently renders wrong (RE-048). RE-086 measured what those 172 scripts actually animate: **71% cycle `PaletteID`** (which of a texture's several palettes is bound — cheap on the PSP GE via `sceGuClutLoad`, no combiner/vertex work needed), 22% cycle `TextureIDCurrent` (frame swapping), a meaningful minority animate UV translate/scale/scroll, and **under 2% touch colour** (`PrimColor`/`EnvColor`/`BlendColor`) — the opposite of what this item's original framing (and `matanim.rs`'s existing costume-selection decoder) assumed. `crates/ssb-rom/src/mobj.rs` only reads `MObjSub.palettes[0]`; `palettes[1..]` exist in the ROM and are needed for the dominant case. Implement palette cycling first, not colour.
