...
- [ ] **Combiner coverage — dominant declined shape detected, not yet consumed on device (RE-073).** `crates/ssb-rom/src/mesh.rs` now evaluates a general `(A-B)*C+D` combiner over both cycles and declines to guess at anything it can't resolve, rather than hardcoding a fixed set of modes as this item originally described. RE-073 measured what it actually declines: 79/1360 `SetCombine` commands archive-wide (5.8%) read `ENVIRONMENT`, 91% of those matching `(PRIM-ENV)*TEXEL+ENV` — a texture-driven blend from `ENV` to `PRIM` with no shade term — across 28 files including Link/Ness/Pikachu's own base models. Added `combiner_texture_blend` to detect it and packed it (`pack.rs`'s `flags::TEXTURE_BLEND`, `VERSION` 9→10), mapping exactly to the PSP GE's native `TextureEffect::Blend` at zero VRAM cost, but `psp/src/meshdraw.rs` deliberately does not wire it up yet: doing so needs affected primitives' vertices baked with a flat base colour, and whether any of those vertices are shared with a normally-shaded primitive (which would then be corrupted) hasn't been checked. Remaining work beyond that: enumerating whatever other combiner shapes are still declined archive-wide.
