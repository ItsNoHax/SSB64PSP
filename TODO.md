...
- [ ] **Graphs without material tables — 113 paired, 14 unpaired, and now characterized rather than a black box (RE-077, RE-078, RE-125, RE-153–157).** Concrete, still-open cases include file 86 (`ITCommonObject`)'s one remaining graph (a fifth mechanism, see below) and file 353's third graph (`SpinAttackDObjDesc @ 0x11C0`, a `WPAttributes` not yet typed in the decompilation). RE-153 removes the whole character-select/result emblem cluster; RE-154/155 add static-effect and source-parallel platform pairs; RE-157 adds Kongo Jungle Bird plus Pikachu's two Special2 effects and Ness's PK Thunder Wave from original descriptor records. The remaining untraced graphs are concentrated in stage files and fighters' special-move files; 9 of the 11 remaining real playable fighters (Mario, Fox, Donkey Kong, Samus, Luigi, Jigglypuff, Captain Falcon, Yoshi, Pikachu) have **zero** unpaired graphs of their own. Search candidates remain leads only unless a typed source table independently confirms them.
