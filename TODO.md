...
- [ ] **Filtering — mode match confirmed (RE-124); reconstruction-formula equivalence reopened (RE-218).** RE-124 measured 151/151 real filter commands as `G_TF_BILERP`; PSP `Linear` filtering matches the *mode*. RE-218 found this never compared the N64 RDP's real 3-point reconstruction formula against PSP's four-tap bilinear filter, which are different operations — see `PLAN.md` R2.0/P0a.
- [ ] **`G_SETTILE` field census — `palette`/`line`/`tmem`/`shift_s`/`shift_t` (RE-218, new).** `dl.rs` decodes all five; `mesh.rs`'s `Cmd::SetTile` match arm discards all five via `..`. Not yet censused archive-wide; not assumed to be a bug. See `PLAN.md` R2.0/P1.
- [ ] **PSP power-of-two texture padding vs N64 logical clamp boundary (RE-218, new).** `psp_texture.rs` zero-fills the padding between a texture's logical dimensions and its power-of-two stride; a clamped N64 coordinate should hold the logical edge texel instead. Not yet measured against real content. See `PLAN.md` R2.0/P0b.

### Phase D — Materials (R0.6 / R0.7) (HIGH)
- [ ] **Vertex lighting provenance —** RE-103/RE-105 replaced the old per-primitive majority vote with a per-vertex signal, but `looks_like_unit_normal` remains in the implementation. R2.2/C2 must tie normal/colour interpretation to `G_VTX` load-time state and quantify any fallback.
...
- [ ] **RDP depth compare/write separation —** R2.2/C3 must preserve `Z_CMP`, `Z_UPD` and `ZMODE` independently; `Z_BUFFER` alone is not a complete depth model.
...
- [ ] **Draw order** — R2.2/C4 must replace the current global material sort with adjacent-only merging; broader layered/performance sorting remains R3.
