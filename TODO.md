...
- [ ] **Blending (`translucent`) — detected but deliberately not consumed (RE-069).** The render mode's actual blend equation (not just `FORCE_BL`, which the opaque default also sets) is correctly decoded into `MeshMaterial::translucent` and packed (14.4% of non-default render modes), but `psp/src/meshdraw.rs` does not read the flag: enabling `GuState::Blend` from it turned Dream Land's canopy-highlight surface into a checkerboard, almost certainly the same open dithered-CI4/coverage problem as RE-053's opaque-path finding, not a detection bug (the render mode itself is a real, decomp-verified `XLU` value). Needs RE-053's dithering/coverage question resolved first, or a per-texture measurement of whether the artifact is visible enough to matter elsewhere before enabling broadly.
