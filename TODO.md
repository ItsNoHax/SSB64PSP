...
- [ ] **Graphs without material tables — 104 paired, 23 unpaired, and now characterized rather than a black box (RE-077, RE-078, RE-125, RE-153, RE-154).** Concrete, still-open cases include file 86 (`ITCommonObject`)'s one remaining graph (a fifth mechanism, see below), file 353's third graph (`SpinAttackDObjDesc @ 0x11C0`, a `WPAttributes` not yet typed in the decompilation), and Ness's one gap (a suggestive but not conclusive candidate). RE-153 removes the whole character-select/result emblem cluster: the original's parallel `dobjdescs[]`/`mobjsubs[]` arrays pair all ten file-35 graphs directly. RE-154 pairs four file-84 common effects from their static `EFDesc` records, with relocData-confirmed table addresses. The remaining untraced graphs are concentrated in effects, stage files and fighters' special-move files; 9 of the 11 remaining real playable fighters (Mario, Fox, Donkey Kong, Samus, Luigi, Jigglypuff, Captain Falcon, Yoshi, Pikachu) have **zero** unpaired graphs of their own. Search candidates remain leads only unless a typed source table independently confirms them.
