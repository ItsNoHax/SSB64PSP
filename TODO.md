...
- [ ] PSP-1000 test
- [x] PSP-2000/3000 test — PSP Slim/6.61 matrix and full 26,254,608-byte
  `MEMSIZE=1` load (RE-201–215, RE-234–260)
- [ ] Long-duration stability test

PPSSPP regression testing remains the day-to-day verification loop used
throughout R0/R1/R2; physical captures now exist for representative current-
renderer scenes, but the formal R2 matrix is still open.
...
**Question:** Why did the canopy remain wrong after mipmaps?
**Status:** Resolved for the documented deterministic scene by RE-067/070/075
and direct physical-PSP capture RE-201. Traditional N64 LOD/mip blending is
separately measured absent (RE-127); generated lower levels remain an
intentional PSP anti-aliasing resource.
**Target:** Closed under R0.5; do not reopen without contradictory hardware
evidence.
...
- [ ] **Wrap/Clamp — verified correct for the mask-nonzero case (RE-066); `mask == 0` and beyond-mirrored-pair coordinates reopened (RE-218).** `psp/src/meshdraw.rs` hardcodes `sceGuTexWrap(Repeat, Repeat)`; measured archive-wide that every tile-0 `G_SETTILE` requesting clamp also has that axis's own mask nonzero (0/754 counterexamples), and cross-checked against `refs/BattleShip`'s interpreter (which strips `G_TX_CLAMP` under the same condition on real hardware) — `mesh.rs`'s existing mask-narrowed width (RE-044) makes `Repeat` exactly correct here, no code change needed. RE-218 found `mesh.rs`'s `clamp_s = cm_s & 0x2 != 0` applies unconditionally on `mask`, and N64's own `mask == 0` semantics (tile bounds define the clamped region directly) were never independently verified — see `PLAN.md` R2.0/P0b.
- [ ] **Mirror — fixed by pre-baking for the cases investigated (RE-067); universality beyond the first mirrored pair reopened (RE-218).** `G_TX_MIRROR` is set on 187/638 (29%) packed textures and has no PSP GE equivalent (`Repeat`/`Clamp` only). `crates/ssb-rom/src/texture.rs::mirror_extend` doubles the decoded image on each mirrored axis (bouncing, not repeating) before `romtool` packs it, so a plain hardware `Repeat` reproduces a real mirror exactly. Traced to Dream Land's canopy specifically and confirmed via a reversible on-device `Repeat`-vs-`Clamp` A/B that the wrap boundary was visibly wrong. Costs +296 KiB VRAM (763→1059 KiB, 1.5x the ~700 KiB budget) — shipped deliberately, per user decision, making texture streaming (below) non-optional. RE-218 found this was never checked against a full N64 tile-addressing reference model for coordinates reaching past the mirrored pair (authored UVs, not just texgen) or against PSP's zero-filled power-of-two texture padding — see `PLAN.md` R2.0/P0b.
...
- [x] **Mipmap/LOD** — RE-127 measured `G_TL_TILE`/`G_TD_CLAMP` for all real commands, so SSB64 does not use traditional LOD/mip blending; the PSP binds level zero. RE-201's direct PSP capture closes the Dream Land canopy acceptance item. Generated lower levels remain an intentional anti-aliasing resource.
- [x] **Dither smoothing — measurable fidelity treatment (RE-070, blur/mirror order corrected RE-075).** Tested RE-053's two suggested approaches: filtering alone (measured on-device, insufficient) and conversion-time dither resolution (box-blurring + packing unquantized as `Psm8888` instead of requantizing to the CI4 palette). The second provides ~40% less adjacent-pixel noise on Dream Land's two canopy textures, confirmed by objective pixel measurement. Implemented as `crates/ssb-rom/src/texture.rs::box_blur_wrapped`, applied through `tools/romtool/src/main.rs`'s `NEEDS_DITHER_BLUR` — a short, named, per-texture allowlist, not a general heuristic. RE-201's direct PSP capture closes the documented Dream Land canopy acceptance; this remains an allowlisted PSP fidelity treatment, not a general texture transform. RE-075 found and fixed the blur/mirror ordering bug and confirmed the packed-byte change.
- [x] **Filtering — accepted measured deviation (RE-219).** All real commands select `G_TF_BILERP`, but the RDP's three-point reconstruction is not PSP four-tap bilinear. Archive-wide error was measured; PSP `Linear` is the closest fixed-function mode.
- [x] **`G_SETTILE` field census (RE-223/224).** Archive-wide census pinned shift/TMEM invariants and found the real CI4 palette-bank gap; bank selection now reaches conversion and has direct tests.
- [x] **PSP power-of-two padding vs N64 clamp boundary (RE-220/222).** Measured archive-wide and fixed by repeating the logical edge texel into padding; POT and mirrored axes remain unchanged by construction.

### Phase D — Materials (R0.6 / R0.7) (HIGH)
- [x] **Vertex lighting provenance (RE-241–243).** `G_VTX` load-time state now determines normal/colour interpretation; the former heuristic fallback was quantified and removed from ownership decisions. Fighter external lighting seeds were checked against source and measured.
...
- [x] **RDP depth compare/write separation (RE-244–251).** `Z_CMP`, `Z_UPD` and `ZMODE` are preserved independently through pack/runtime; the dedicated diagnostic pins the required depth-write ON→OFF→ON sequence.
...
- [x] **Graphs without material tables — all 127 paired (RE-163).** File 251's typed N-Bumper pair resolves file 86; Link's passive-part dispatch supplies file 324's final typed relationship. The archive-wide pairing census has zero remaining mismatches.
...
- [x] **Draw order (RE-252).** Global material regrouping was replaced with adjacent-only merging; non-adjacent `A B A` runs retain submission order. Broader performance sorting remains R3 and must preserve this baseline.
...
- [ ] **Fighter costume palettes** — Costume zero now resolves palette ID and all five colour/light tracks (RE-096, RE-261), but runtime selection and packing all costumes remain future work.
