...
- [ ] **Graphs without material tables — 125 paired, 2 unpaired (RE-161).** RE-160 source-pairs Sector Z's Ship, Final Destination's four wallpaper effects, and Dream Land's Whispy eyes, leaving only file 86 (`ITCommonObject`)'s N-Bumper graph and file 324's Link-model `JointTree_0x9CF8`. RE-161 audited all three Link candidates against the decompilation: none is named for that graph, so demand-compatible tables remain leads only unless a typed source table independently confirms them.
