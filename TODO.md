...
- [ ] **A third, uncaught flat-colour combiner shape (RE-079).** The archive-wide census also found `(ZERO-ZERO)*ZERO+PRIM` — a plain constant `PRIMITIVE` colour with no shade or texture dependence at all — for 1,589 primitives, which neither `combiner_shade_scale` nor `combiner_texture_blend` models (the former needs an `s`/`st` term, the latter needs both a constant *and* a texel term). Needs a new `MeshMaterial` field (e.g. `flat_color: Option<[u8;4]>`) and device-side handling, not a classification fix to an existing function.
