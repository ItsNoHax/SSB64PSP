...
- [ ] **Alpha compare / render mode / combiner defaults** — leads from RE-068, not yet acted on: the same reset list sets `G_AC_NONE` (alpha compare off by default), `G_RM_OPA_SURF`/`G_RM_OPA_SURF2` (opaque, no blending, by default) and `G_CC_SHADE`/`G_CC_SHADE` (shade-only combiner by default) as the real starting state for every object, the same way `Z_BUFFER`/`CULL_BACK`/`SHADING_SMOOTH` were. Worth checking whether `mesh.rs`'s current "unset means declined/unmodified" handling for these already happens to coincide with these real defaults (as it did for the combiner, coincidentally) or needs the same kind of fix.
