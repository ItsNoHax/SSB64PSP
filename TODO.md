...
- [x] **Shading-detection majority vote — replaced by a per-vertex, data-driven decision (RE-103/RE-105), stale item corrected.** This item previously described `pack::looks_like_unit_normal` plus an 80% *per-primitive* vote (RE-021) as still needed; that was true when written but is now stale — `PLAN.md` R0.6 records that RE-103 found the per-primitive majority vote wrong by construction (a fighter's mixed material routinely splits 20–80% within one primitive) and replaced it with a per-*vertex* decision, and RE-105 gave that per-vertex decision a real ROM-derived signal (`G_MW_LIGHTCOL`, confirmed in `crates/ssb-rom/src/mesh.rs`) instead of only `looks_like_unit_normal`'s heuristic shape check (still present, `crates/ssb-rom/src/pack.rs:936`, but no longer the sole/majority-vote input). This is not full correctness — see the light-*direction* note below, unchanged — but the specific "majority vote" gap this bullet named is closed.
- [ ] **Light direction** — `pack.rs`'s baked `LIGHT_DIR` uses the real, measured, ROM-derived `(20, 45)` degree angle that 80% of stages share (RE-065) instead of an arbitrary constant, but it is still one baked value for the whole pack, not per-stage. Full correctness (varying the light per stage, and per-object `G_LIGHTING` colours) needs runtime `sceGuLight` lighting, not pack-time baking — tracked as an accepted deviation (`PLAN.md` R0.6, R0.16), not attempted here.
...
- [ ] **Graphs without material tables — 70 paired, 57 unpaired, and now characterized rather than a black box (RE-077, RE-078).** Freshly re-measured (`romtool mobj`, no `--file` filter) after RE-059/RE-060's 7 fixes, RE-077's 1, and RE-078's 6 more. Concrete, still-open cases: file 86 (`ITCommonObject`)'s one remaining graph (a fifth mechanism, see below), file 353's third graph (`SpinAttackDObjDesc @ 0x11C0`, a `WPAttributes` not yet typed in the decompilation), and Ness's one gap (a suggestive but not conclusive candidate) — see RE-057/RE-058/RE-060/RE-077. RE-077 checked, rather than assumed, whether the untraced graphs are hiding in core fighter models: they mostly aren't. 9 of the 11 remaining real playable fighters (Mario, Fox, Donkey Kong, Samus, Luigi, Jigglypuff, Captain Falcon, Yoshi, Pikachu) have **zero** unpaired graphs of their own; the untraced ones are concentrated in character-select emblem models, stage files, and fighters' special-move files instead. Kirby's one core-model gap (22-node `JointTree_0x19F08`) and RE-078's six more (menu/opening-movie/shared-effect files) were each found via `--search` landing on exactly one demand-matching candidate (not the usual dozens) and confirmed against a fully-typed decompiled symbol, address *and* entry count both agreeing with the graph's own node count, before fixing — a looser substring-matched "confirmation" of two other candidates was caught and dropped when a stricter, address-anchored check found nothing really at that address.
...
- [ ] **Stage material animation — decoder exists and is tested (RE-087); pack format and device wiring do not yet.** Hook `matanim_joints` into `draw_stage_animated` (12 layers, 172 individual scripts, `AObjEvent32`). RE-086 measured what those 172 scripts actually animate: **71% cycle `PaletteID`** (which of a texture's several palettes is bound — cheap on the PSP GE via `sceGuClutLoad`, no combiner/vertex work needed), 22% cycle `TextureIDCurrent` (frame swapping), a meaningful minority animate UV translate/scale/scroll, and **under 2% touch colour** — the opposite of what this item's original framing assumed. RE-087 added `matanim::MaterialJoint`, a persistent tick-based decoder (unlike the existing `colors_at`, built only for fighter costume selection) covering all of this, including the `JUMP`/`SET_ANIM` looping a real `PaletteID` script actually uses. Remaining: `crates/ssb-rom/src/mobj.rs` still only reads `MObjSub.palettes[0]` (`palettes[1..]` exist in the ROM and are needed for the dominant case); nothing resolves `p_matanim_joints` into per-(node, `MObj`) script references at pack time; no pack table carries any of this; nothing on the device side ticks a `MaterialJoint` or reloads a CLUT.
