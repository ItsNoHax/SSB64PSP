...
- [ ] **Graphs without material tables — 125 paired, 2 unpaired (RE-160).** RE-160 source-pairs Sector Z's Ship, Final Destination's four wallpaper effects, and Dream Land's Whispy eyes, leaving only file 86 (`ITCommonObject`)'s N-Bumper graph and file 324's untyped Link-model graph. Search candidates remain leads only unless a typed source table independently confirms them.
