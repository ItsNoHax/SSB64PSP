...
- [ ] **Graphs without material tables — 119 paired, 8 unpaired, and now characterized rather than a black box (RE-077, RE-078, RE-125, RE-153–159).** Concrete, still-open cases include file 86 (`ITCommonObject`)'s one remaining graph (a fifth mechanism, see below). RE-153 removes the whole character-select/result emblem cluster; RE-154/155 add static-effect and source-parallel platform pairs; RE-157 adds Kongo Jungle Bird plus Pikachu's two Special2 effects and Ness's PK Thunder Wave; RE-158 adds Link Spin Attack, Samus Grapple Beam, Captain Falcon Kick, Jigglypuff Sing and Ness Psychic Magnet; and RE-159 adds the explanation screen's control-stick interface from a source call sequence. The remaining untraced graphs are concentrated in stage files and the N-Bumper item. Search candidates remain leads only unless a typed source table independently confirms them.
