...
- [ ] **A fifth mechanism, byte-offset delta — file 86's last graph (RE-060), not yet fixed.** `refs/ssb-decomp-re/src/it/itcommon/itnbumper.c:367`'s `itGetPData(ip, &llITCommonDataNBumperDataStart, &llITCommonDataNBumperWaitMObjSub)` computes an MObjSub pointer as a compile-time byte-offset delta added to a runtime-resolved base pointer — neither a struct field nor a call-sequence pairing. Not confirmed which table file 86's unpaired graph (`0x7BE8`, `..._gap_0x76CC_sub_0x42C_post`) actually needs, or whether it matches the "NBumperWait" state `itnbumper.c` names; left unfixed rather than guessing.
