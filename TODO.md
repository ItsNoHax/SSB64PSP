...
- [ ] **Runtime `MObj` display-state parity** — reproduce the complete
  `gcDrawMObjForDObj` path in `refs/ssb-decomp-re/src/sys/objdisplay.c:1150-1410`:
  ...
  current/next texture plus fractional-alpha state. `MOBJ_FLAG_FRAC` and UV
  Scroll below are focused implementation slices of this parent gap; the
  parent remains open until those slices are connected to one source-backed
  runtime state model.
