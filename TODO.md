...
- [x] **MissingPalette (4→3) — root-caused, moved to R0.7; one of three files fixed.** RE-057 (via a temporary instrumented trace, reverted) confirmed the mechanism: files 52 (`MVCommon`) and 353 (`LinkSpecial2`) get **zero** `MObj` materials from `PartTables` for their scene graphs, and file 86 (`ITCommonObject`) gets them for most but not all nodes. Every segment-0x0E `Call` mesh.rs can't resolve calls `forget_texture()`, which clears an otherwise-valid palette a nearby `G_LOADTLUT` had just set. Not a `romtool` dedup artifact (RE-056's earlier guess). RE-058/RE-059 then traced and fixed two of file 353's three unpaired graphs (Link's entrance effects, paired via a third record shape — `EFDesc` — hand-entered in `tools/romtool/src/main.rs`'s `load_all` since it lives outside the archive). `romtool textures` now reports 3 `MissingPalette`, not 4. Files 52, 86 and 353's third graph (a `WPAttributes`-named Spin Attack model) remain open — see the `WPAttributes`/`EFDesc` bullet below.
...
- Downgraded from failure to informational: **TLUT state across lists** — was 28 "CI texture, no TLUT recorded" failures, now only 3, and those 3 are reported as a *note* on textures that still pack successfully, not as conversion failures. Confirm whether the remaining 3 need action or are benign.
