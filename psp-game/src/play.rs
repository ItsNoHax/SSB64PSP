...
//!
//! Verbatim copy of `psp/src/play.rs`, per `psp-game`'s crate-boundary rule
//! (own copy, not a shared refactor -- `plans/gameplay/F1.md`). `psp-game`'s
//! training scene is exactly this slice, driven by real input instead of a
//! debug viewer's inspection camera.
//!
