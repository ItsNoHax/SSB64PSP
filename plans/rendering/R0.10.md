...
Status: `COMPLETE` — RE-301 implements packed stage `TextureIDCurrent` and
tile-0 UV tracks without replacing the working palette path, and records
every remaining used track as verified inert or a documented PSP limitation.
The historical 172-script census is not reproducible and is no longer an
acceptance count.
...
RE-301 re-ran the census against the current ROM/archive loader: 61 resolvable
stage attachments replay without failure (14 `TextureIDCurrent`, 15 `TraU`,
13 `TraV`, 2 each scale/scroll/fractional shapes, 33 `PaletteID`). The
historical 172 count came from a reverted temporary walker and remains
evidence debt until it can be reproduced; it must not be claimed as runtime
coverage.
...
* [x] stage material runtime — RE-301 packs all resolved texture frames and each MObj's rest UV state, resolves `TextureIDCurrent`, and applies the original tile-0 translate/scale delta while preserving palette animation. `ScrU`/`ScrV` are verified tile-1-inert; `SetLFrac`/`TextureIDNext` are the documented two-tile RDP limitation.
* [x] targeted PPSSPP acceptance — RE-301 rebaselines deterministic Dream Land UV and Meta Crystal `TextureIDCurrent` captures; repeat captures are byte-identical. The Link costume-1 golden proves the existing PaletteID path selects a non-zero variant.
* [x] fighter costume data — RE-096/RE-097 pack `PaletteID` with the existing costume colour/light path; the viewer can select non-zero variants. The player-facing selector still chooses costume zero, a front-end gap rather than a parallel costume renderer.
* [x] complete track accounting — RE-301 replays all 61 currently resolvable attachments with zero failures. Tile-1 scroll has no `TEXEL1` consumer; fractional two-image blending and sparse stage colour/light tracks are documented PSP limitations, not silently ignored.
