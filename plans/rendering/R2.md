...
neutral-light/single-cycle-combiner algebra, root cause is now narrowed
specifically to the PSP runtime texture/CLUT bind path
(`psp/src/meshdraw.rs`), not any part of `crates/ssb-rom`/`tools/romtool`.
Root cause not yet found for Yoshi, Captain Falcon, Ness, Pikachu, Kirby
either. Keep R3 blocked until RE-283 concludes for all of them, in addition
to the PSP-1000/coverage items above.
