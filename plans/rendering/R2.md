...
* [ ] no hardware-only rendering failures remain — five golden scenes (Dream Land/Mario, `MVOpeningRoom`, `StageSectorFile2`, `CatchSwirl`, Saffron City) plus fighter, texgen, and plain-interactive checks are clean. RE-265 adds exact PPSSPP software goldens for all 12 playable fighters in their high-detail Wait pose with runtime fighter lighting; RE-270 physically re-confirms Fox/Ness/Link/Donkey Kong/Dream Land against RE-262/264/269's fixes; RE-271 physically confirms the remaining 6 playable fighters, re-confirms Kirby, physically runs the depth-mask diagnostic for the first time (finding and fixing a real `Gpu::draw_triangles` GE race along the way), and sustains 10 continuous minutes with zero exceptions; RE-273 repeats the depth-mask-diagnostic and 10-minute sustained checks with zero exceptions on a second physical unit (PSP-3000) — all 22 committed deterministic goldens are now physically confirmed on two units. RE-284 extends the sustained-run sample to 30 continuous minutes (one unit, PSP Slim): zero exceptions across six 5-minute checkpoints, t=0/t=30 native captures differing only in a tiny Mario-idle-pose region (43 px), no corruption or leak. Coverage across all 41 stages/effects (no dedicated capture scenes exist for most of them), a second unit at the 30-minute duration, and PSP-1000 remain open, deliberately deferred until the game structure grows beyond the current asset viewer. Separately, RE-272 found a real (not hardware-only — reproduces on PPSSPP too) fighter-face-texture rendering bug during RE-273's Mario capture; RE-280 root-caused it, RE-281 fixed it and refreshed the affected goldens on PPSSPP software rendering, and RE-282 physically re-confirmed the fix on a PSP Slim — this bug's chain is now fully closed on both platforms
