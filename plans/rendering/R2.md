...
own refreshed golden). Physical re-confirmation of this second fix was
blocked this session by a USB-permission environment issue (device enumerates
but `usbhostfs_pc` cannot claim it), not attempted — a follow-up, not a
concluded gap, given the underlying mechanism was already physically proven
for the boot instance of the same texture shape. Root cause not yet found for
Yoshi, Captain Falcon, Ness, Pikachu, Kirby. Keep R3 blocked until RE-283
concludes for all of them (including physical re-confirmation of the shin-cuff
fix), in addition to the PSP-1000/coverage items above.
