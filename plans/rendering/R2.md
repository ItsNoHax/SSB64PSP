...
named fighters; for Fox, the wrist-node defect colour is fully traced to
its own ROM-authored `SetCombine`/`SetPrimColor` and the conversion
pipeline confirmed correct (not a porting bug) — only whether real N64
hardware also draws it remains open. One hypothesis (CI4 swizzle-threshold)
tested and rejected. Keep R3 blocked
until RE-283 concludes, in addition to the PSP-1000/coverage items above.
