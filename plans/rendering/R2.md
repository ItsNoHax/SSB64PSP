...
which is what RE-283 measures against. Root cause not yet found for 8 of 9
named fighters; Fox's item is now fully closed — the wrist-node defect
colour traced to its own ROM-authored `SetCombine`/`SetPrimColor`, the
conversion pipeline confirmed correct, and a live original-N64 capture
(`n64-emulator` Skill) confirmed real hardware draws the identical brown
wrist band, visibly, in the same idle pose — not a porting bug. One
hypothesis (CI4 swizzle-threshold) tested and rejected. Keep R3 blocked
until RE-283 concludes for the remaining 8 fighters, in addition to the
