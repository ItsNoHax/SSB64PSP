...
medium-high rather than Fox's high confidence. Samus's item is narrowed
(two candidate ROM-authored CI4 textures identified, both decode cleanly
from ROM, so not a corrupted-texel bug) but not yet pinned to the exact
defect pixels — still open. One hypothesis (CI4 swizzle-threshold) tested
and rejected. Root cause not yet found for 6 of 9 named fighters (Samus,
Link, Yoshi, Falcon, Ness, Pikachu, Kirby). Keep R3 blocked until RE-283
concludes for all of them, in addition to the PSP-1000/coverage items
above.
