...
chest" is positively traced to node 12 rendered in its real Wait-animation
pose (a posed-isolate diff against the golden matches 92.7% of compared
pixels, remainder at silhouette anti-aliasing) and is structurally
consistent with real, correctly-bound `TEXEL * SHADE` combiner math on a
real ROM texture rather than a stray read or corruption (medium confidence
— not yet exactly reconstructed or hardware-checked the way Fox's item was);
her thigh speckle does not reproduce at this camera distance. One hypothesis
(CI4 swizzle-threshold) tested and rejected. Root cause not yet found for
Link, Yoshi, Captain Falcon, Ness, Pikachu, Kirby. Keep R3 blocked until
