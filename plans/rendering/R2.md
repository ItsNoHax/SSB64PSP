...
Status: `COMPLETE` — R2.0, R2.1 and R2.2 are complete. Representative
...
owed. RE-288 then physically tested a PSP-1000: the pack (~25.6 MiB) fails
to fit in its 32 MiB RAM, since `MEMSIZE=1` is ignored on that hardware
class (unlike the 64 MiB PSP-3000 RE-273 tested) — `assets::load_pack`
returns `LoadError::OutOfMemory` cleanly (confirmed via a new one-shot,
GU-free `pack_status_overlay` diagnostic feature added this session; the
existing `debug_overlay` HUD's `sceGuDebugFlush` path was suspected of a
hardware-only crash on this unit but that turned out to be stale PSPLink
exception state from an unreset prior `kill`, not a real fault — see RE-288)
and the game falls back to its built-in placeholder tetrahedron with zero
exceptions, not a rendering bug. This closes the open "is PSP-1000 pack
compatibility unresolved" question with a real, documented answer. Per
explicit user instruction, further PSP-1000 hardware work (a build that fits
its RAM class) and a second physical unit at the 30-minute sustained-run
duration are moved out of R2 entirely and deferred to a future hardware-
acceptance pass once real game UI/scene loading replaces the current debug
asset viewer (see `TODO.md`) — neither blocks R2's own closure or R3.
...
code. **Now fixed, not an `ACCEPTED_DEVIATION`:** a stride/swizzle-floor
hypothesis was tested and rejected (widening the row enough to make the
texture swizzle left the defect byte-identical), then the fix that already
worked for Donkey Kong's own paletted-texture failure (RE-267 — bypass PSP's
CLUT/palette transport, ship decoded RGBA instead) was applied scoped to
this one texture (`tools/romtool`'s `convert_texture`, `is_link_boot`).
Confirmed removed on PPSSPP (node-isolated and full-pose capture, 21/22
other deterministic goldens unaffected) and on physical PSP hardware, at the
same bar the defect itself was confirmed. A second, separate defect was then
found on Link's shin cuff/greave (node 23 of the same runtime graph, one leg
only — its mirrored counterpart, node 28, was already clean): the same
texture, bound with `Wrap` addressing on one leg's display list and `Mirror`
on the other, and the same 8-byte-row `PsmT4` packed shape as the boot
texture. Node-isolated, confirmed the same GE-quirk mechanism, and fixed with
the same paletted-transport bypass (`is_link_shin`), closing it on PPSSPP
(12/12 playable-fighter goldens re-verified byte-identical except Link's
own refreshed golden) and, once the user reconnected the PSP after an
initial USB-permission failure, on physical PSP hardware too (native
`scrshot` of the same node-isolated primitive, clean). Yoshi's head noise
(node 3: two small CI4 decals, both eyes and a mouth-corner mark, both under
`Clamp` addressing rather than `Wrap`) is now closed too, at the same bar,
using the same fix class (`is_yoshi_head`) — new evidence the GE quirk keys
on the small-paletted-texture shape itself, not the wrap mode, confirmed on
PPSSPP (node-isolated, additive per-texture disable/re-enable localizing each
symptom, and full-pose) and on physical PSP hardware (native `scrshot`,
clean). Captain Falcon's shoulder pad (node 10, file 332) is now closed too:
a prior session's two ruled-out leads had both been tested against a
misidentified texture (a desynced raw `dlraw` dump's false offsets); a
follow-up session recovered the node's real 8x8 CI4 dither texture (offset
`0xC508`) from the authoritative `pack()`/`plan_draw_order` pipeline and
found the same `Psm8888` bypass, applied to the correct texture this time,
removes the defect completely — confirmed on PPSSPP (node-isolated and
full-pose) and physical PSP hardware. Ness's shoulder-strap noise (node 4,
graph 0x26B0, an 8x8 CI4 texture at offset 0xBDE8 mirror-narrowed to 8x16 —
recovered from the authoritative pack()/plan_draw_order pipeline after a
raw dlraw-style decode gave a false lead) is now closed too, at the same
bar, using the same fix class (is_ness_shoulder), confirmed on PPSSPP
(node-isolated and full-pose) and physical PSP hardware. **Pikachu and
Kirby are now traced too, and close with no code fix:** node-isolating
both fighters' default Wait-pose graphs found neither reproduces the
scrambled-CI4 defect class. Pikachu's "cracks" traces to a genuine,
uncorrupted ROM gradient decal (file 341 `+0x7350`) rendered via correct
clamp-overflow addressing — the `Psm8888` bypass was tested and made zero
pixel difference, a clean negative result explained by the primitive's own
UV span rather than an unexplained one; its "face colour mismatch" is not
present in the current build. Kirby's only reachable body texture (a face
decal, file 328 `+0x1CF60`) matches its raw ROM data exactly, and its
arms/feet carry no texture at all, so nothing in this defect class could
apply; the file's other, stranger textures all belong to Kirby's
per-copy-ability hat graphs or non-default costumes, none reachable from
the tested costume-0 body. This closes RE-283's per-fighter chase — all
nine originally reported items are now accounted for (eight fixed and
hardware-confirmed, two traced to no reproducible defect). R3 is not
blocked by RE-283, nor by the PSP-1000/second-unit items, which are now
deferred out of R2 (see above and `TODO.md`).
...
* [x] no hardware-only rendering failures remain — five golden scenes (Dream Land/Mario, `MVOpeningRoom`, `StageSectorFile2`, `CatchSwirl`, Saffron City) plus fighter, texgen, and plain-interactive checks are clean. RE-265 adds exact PPSSPP software goldens for all 12 playable fighters in their high-detail Wait pose with runtime fighter lighting; RE-270 physically re-confirms Fox/Ness/Link/Donkey Kong/Dream Land against RE-262/264/269's fixes; RE-271 physically confirms the remaining 6 playable fighters, re-confirms Kirby, physically runs the depth-mask diagnostic for the first time (finding and fixing a real `Gpu::draw_triangles` GE race along the way), and sustains 10 continuous minutes with zero exceptions; RE-273 repeats the depth-mask-diagnostic and 10-minute sustained checks with zero exceptions on a second physical unit (PSP-3000) — all 22 committed deterministic goldens are now physically confirmed on two units. RE-284 extends the sustained-run sample to 30 continuous minutes (one unit, PSP Slim): zero exceptions across six 5-minute checkpoints, t=0/t=30 native captures differing only in a tiny Mario-idle-pose region (43 px), no corruption or leak. RE-285 adds a fourth stage golden, Peach's Castle (scene 10, stage 4/file 259) — deterministic, zero regression on the other 22+ goldens, and physically confirmed with zero exceptions. RE-286 adds deterministic PPSSPP goldens for all remaining 37 stage entries (the true remaining count, corrected from an earlier undercount that missed Mushroom Kingdom/Beta Dream Land/Test Stage/Duel Zone/Small Yoshi's Island — see RE-286), closing software-side coverage of all 41 stages/effects. RE-287 then physically confirms all 37 on real PSP hardware in one batched PSPLink pass: zero exceptions on every one of the 37 loads, no blank/locked captures (391,734-byte native frames, 360-6,040 unique colors each), and 8 of the 37 spot-checked pixel-exact against their PPSSPP golden spanning every stage category in the batch — stage coverage is now 41/41 confirmed on both PPSSPP and physical hardware. RE-288 physically tests a PSP-1000: its pack load fails cleanly with `LoadError::OutOfMemory` (32 MiB RAM, `MEMSIZE=1` ignored on this hardware class, ~25.6 MiB pack doesn't fit), falling back to the built-in placeholder with zero exceptions — a real hardware/RAM-class constraint, not a rendering bug, and not a claim R2 makes. A second unit at the 30-minute sustained-run duration, and a PSP-1000 run against real game content once a build fits its RAM class, are deferred out of R2 entirely to a future hardware-acceptance pass once real game UI/scene loading replaces the current debug asset viewer (see `TODO.md`); this acceptance item is about hardware-only rendering failures, which RE-288 confirms none of on PSP-1000 either. Separately, RE-272 found a real (not hardware-only — reproduces on PPSSPP too) fighter-face-texture rendering bug during RE-273's Mario capture; RE-280 root-caused it, RE-281 fixed it and refreshed the affected goldens on PPSSPP software rendering, and RE-282 physically re-confirmed the fix on a PSP Slim — this bug's chain is now fully closed on both platforms
