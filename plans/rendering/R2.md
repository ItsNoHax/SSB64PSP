...
both feet) and its combiner decoded exactly as `(TEXEL0*SHADE)*ENVIRONMENT`,
but the root cause is not yet found — a hand re-derivation of `mesh.rs`'s
own combiner algebra predicts a clean, untinted result for this shape,
contradicting the observed render, so a live-state or material-animation
mechanism remains to be checked. Root cause not yet found for Yoshi, Captain
Falcon, Ness, Pikachu, Kirby either. Keep R3 blocked until RE-283 concludes
for all of them, in addition to the PSP-1000/coverage items above.
