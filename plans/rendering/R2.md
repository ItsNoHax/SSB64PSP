...
closing this bug on PPSSPP software rendering; a dedicated physical-hardware
recapture of the fix remains part of the physical matrix below. The formal
hardware matrix still lacks PSP-1000 (its 32 MiB RAM cannot use `MEMSIZE=1`,
unlike the 64 MiB PSP-3000 RE-273 tested) and coverage beyond the 22
committed deterministic goldens; those remain deliberately deferred until the
game structure grows beyond the current asset viewer, and keep R3 blocked
until resumed.
