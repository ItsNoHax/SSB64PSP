...
rendering bug (not hardware-specific, not caused by this session, not yet
root-caused) that the regression-capture golden methodology cannot detect by
construction. The formal hardware matrix still lacks PSP-1000 (its 32 MiB
RAM cannot use `MEMSIZE=1`, unlike the 64 MiB PSP-3000 RE-273 tested) and
coverage beyond the 22 committed deterministic goldens; those remain
deliberately deferred until the game structure grows beyond the current
asset viewer, and keep R3 blocked until resumed. RE-272's texture bug is a
separate, scoped correctness follow-up (`TODO.md`), not itself a blocker to
resuming the physical matrix, but it means "representative fighters render"
below should not be read as pixel-fidelity-to-the-original-texture proof.
